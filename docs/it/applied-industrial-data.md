...
| --- | --- | --- | --- | --- |
| AI·데이터 공통 기초 | 단과대학 공통 | Python·데이터, 생성형 AI 활용, ML 기초, AI 윤리 | 데이터를 다루고 ML 모델과 생성형 AI 도구를 책임 있게 활용 | 데이터리터러시, 공학프로그래밍, 인공지능의 이해와 활용, 생성형 AI와 비즈니스 애널리틱스, AI 윤리(제안) |
| 산업통계·OR 기초 | 학부 공통 | 확률·통계, 최적화, 의사결정론 | 산업 문제를 수리모형·통계로 해석 | 공학수학과 통계, 데이터공학 선형대수, 경영과학I, 경영과학II, 최적화이론(제안) |
| 시스템 모델링·시뮬레이션 | 학부 공통 | 이산사건 시뮬레이션, 시스템 다이내믹스 | 복잡 시스템을 모델링·시뮬레이션으로 분석 | 시뮬레이션(제안), 시스템모델링(제안) |
| 데이터 엔지니어링 | 트랙 전공 | 데이터베이스, ETL, 빅데이터 처리 | 산업 데이터 수집·정제·저장 파이프라인 구축 | 데이터베이스 설계 및 활용, 데이터 시각화와 비즈니스 인텔리전스, 빅데이터처리(제안), 데이터엔지니어링(제안) |
| 산업 머신러닝·예측분석 | 트랙 전공 | 시계열·ML·딥러닝, 수요예측·이상탐지 | AI 기반 예측·탐지 모델 개발·평가 | 인공지능의 이해와 활용, 머신러닝(제안), AI 수요예측(제안), 이상탐지·예측정비(제안) |
| 디지털트윈·MLOps | 트랙 전공 | 디지털트윈, 모델 배포·운영, 모니터링 | 산업AI 모델을 서비스로 배포·운영 | 디지털트윈(제안), MLOps(제안), 산업AI서비스(제안) |
| 산업데이터 캡스톤 | 트랙 전공 | 융합 종합설계, 산학 프로젝트 | 데이터 파이프라인부터 배포까지 통합 구현 | 산업데이터공학종합설계(캡스톤)(제안) |
