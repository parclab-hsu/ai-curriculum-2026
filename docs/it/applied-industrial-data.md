...
!!! info "진출 직무 설명 — 이 직무는 어떤 일을 하나요?"
    ...
    - **산업 데이터 엔지니어 (경로 A):** 공장·물류 현장에서 쏟아지는 데이터를 모으고 정리해 언제든 분석할 수 있게 통로(파이프라인)를 만드는 일이다. 데이터의 흐름과 저장을 책임진다. → *AI 활용:* AI·생성형 도구로 데이터 처리 코드 작성과 품질 점검을 자동화해 파이프라인을 효율적으로 만든다.
    - **산업 데이터 사이언티스트 (경로 B):** 쌓인 산업 데이터에서 의미 있는 패턴을 찾아 예측·분석 모델을 만드는 일이다. 수요예측·이상탐지 같은 문제를 데이터로 푼다. → *AI 활용:* 머신러닝·딥러닝 모델을 직접 만들어 미래를 예측하고 설비 이상을 미리 잡아낸다.
    - **스마트팩토리 MLOps 엔지니어 (경로 C):** 만든 AI 모델을 실제 공장 시스템에 올려 안정적으로 돌아가게 배포·운영하는 일이다. 성능이 떨어지면 다시 학습시켜 유지한다. → *AI 활용:* 모델 성능을 자동으로 모니터링하고 데이터 변화(드리프트)를 감지해 재학습을 자동으로 돌린다.
    - **물류·SCM 수요예측 분석가 (경로 D):** 얼마나 팔리고 필요할지 데이터로 예측해 재고·배송 계획으로 연결하는 일이다. 물류비를 줄이고 결품을 막는다. → *AI 활용:* 시계열·머신러닝 예측 결과를 최적화와 연결해 재고·배송 의사결정을 자동으로 제안한다.

### 10-5. 상위 수준 보완 권고

> 아래는 KAIST·성균관대·한양대 산업/데이터공학 등 산업데이터·제조AI 특성화 **상위 비교군** 및 산업 표준 정렬을 위한 **보완 권고**다. **공식 교과를 대체하지 않으며**, 2027학년도 교과 개편 시 심의 의견·향후 개선 계획으로 활용한다.
