...
모바일소프트웨어트랙은 안드로이드(Kotlin)·iOS(Swift) 네이티브 앱과 Flutter·KMP(Kotlin Multiplatform) 등 크로스플랫폼 모바일 소프트웨어를 설계·개발하는 트랙입니다.

**AI 융합 개편 방향**: 2024 생성형 AI → 2025 에이전트 AI → 2026 Physical AI로 이어지는 패러다임 속에서, 모바일은 **온디바이스 AI의 실행 단말**이자 **에이전트의 프런트엔드·센서/액추에이터 게이트웨이**로 역할이 재정의되고 있습니다. 단순 UI 구현을 넘어 '앱에 LLM 기능을 직접 통합'하는 온디바이스 AI 역량을 정규 커리큘럼에 결합합니다.
...
사람인·잡코리아·원티드·LinkedIn 등 다채널 기준, 채용 규모는 줄고 기준은 높아진 **양극화 시장**으로, 곧바로 투입 가능한 경력자 선호가 강합니다.
...
> 학문 분야 정체성: 모바일소프트웨어트랙은 모바일·임베디드 환경에서 동작하는 소프트웨어를 설계·구현하는 SW공학 역량에 온디바이스 AI를 결합하여, 단말에서 실시간으로 지능형 서비스를 제공하는 엔지니어를 양성합니다.
...
> 출처: 한성대학교 모바일소프트웨어트랙 공식 교과과정([https://www.hansung.ac.kr/Engineering/4889/subview.do](https://www.hansung.ac.kr/Engineering/4889/subview.do)) 기준, 확인일 2026-06-30. 구성 교과목 공식, 미존재 보강은 (제안). (전기=전공기초·전필=전공필수·전선=전공선택)
...
| --- | --- | --- | --- | --- | --- | --- |
| **M1** | AI·데이터 공통 기초 | 컴퓨터프로그래밍(1-2·전기) · 프로그래밍랩(2-1·전선) · 확률및통계(2-1·전선) · AI를 이용한 주식가치평가(1-1·전선) | Python·데이터 처리, 생성형 AI/LLM 활용, AI 코딩 어시스턴트, AI 윤리 | AI 도구로 데이터 처리·프로토타이핑 수행 | 단과대학 공통 | 공식 |
| **M2** | 컴퓨터과학 기반 | 자료구조(2-1·전선) · 컴퓨터구조(2-1·전선) · 알고리즘(2-2·전선) · 운영체제(3-1·전선) | 자료구조, 알고리즘, 운영체제, 네트워크 | 시스템 동작 원리 이해 및 효율적 알고리즘 설계 | 학부 공통 | 공식 |
| **M3** | 소프트웨어공학 기반 | 객체지향언어1(2-1·전선) · 객체지향언어2(2-2·전선) · 소프트웨어공학(3-1·전선) · 설계패턴(3-2·전선) | 객체지향, 협업·형상관리, 테스트 | 협업 기반 SW 개발 프로세스 수행 | 학부 공통 | 공식 |
| **M4** | 모바일 앱 개발 | 모바일&스마트시스템(2-2·전필) · 안드로이드프로그래밍(3-1·전필) · 고급모바일프로그래밍(3-2·전필) · iOS프로그래밍(4-1·전필) | 안드로이드/iOS·Flutter, 모바일 UI/UX, 앱 아키텍처 | 스토어 배포 가능한 앱 독립 구현 | M4-M5-M6 선후수 | 공식 |
| **M5** | 온디바이스 AI | 데이터마이닝(3-1·전선) · 컴퓨터비젼(3-2·전선) · 모바일시스템응용프로젝트(4-2·전필) · 온디바이스AI(제안·미정) | 모델 경량화·양자화, TFLite/Core ML, 엣지 추론 | 단말 내 실시간 AI 기능 탑재 | M4-M5-M6 선후수·마이크로디그리 | 공식·제안·미정 |
| **M6** | 생성형 AI 모바일 서비스 | 네트워크프로그래밍(3-2·전선) · 웹서버프로그래밍(3-1·전선) · 생성형AI앱개발(제안·미정) · 모바일AI서비스(제안·미정) | LLM API·sLM, RAG, 멀티모달 연동 | 대화형 AI 모바일 앱 구현 | M4-M5-M6 선후수 | 공식·제안·미정 |
| **M7** | 모바일 캡스톤 | 모바일 캡스톤디자인(4-2·전필) · 기업연계 SW캡스톤디자인(3-2·전선) · 융합캡스톤디자인(4-2·전선) | 기획·개발·배포·운영, 산학 협업 | AI 모바일 서비스 종합 완성 | 종합·캡스톤 | 공식 |
...
!!! info "진출 직무 — 이런 전문가로 성장할 수 있어요"
    ...
    - **온디바이스 AI 엔지니어 (경로 A):** 서버 없이도 손안의 폰에서 곧바로 돌아가는 AI를 만드는 전문가입니다. 거대한 모델을 손바닥만 한 단말에 넣어 실시간으로 작동시키는, 남들이 어렵다고 하는 일을 해내며 "이걸 폰에서?"라는 감탄을 이끌어 낼 수 있습니다. → *AI 활용 능력·역량:* Gemini Nano·Core ML 경량 모델을 앱에 이식하고 양자화로 속도·배터리를 최적화하는 **온디바이스 AI 경량화·최적화 역량**.
    - **AI 모바일 서비스 개발자 (경로 B):** 챗봇·요약·이미지 설명 같은 AI 기능을 내 손으로 앱에 담아 스토어에 올리는 개발자입니다. 내가 만든 AI 기능을 수많은 사용자가 매일 손끝으로 쓰는 모습을 직접 확인하는 성취를 누릴 수 있습니다. → *AI 활용 능력·역량:* LLM API와 RAG를 연동해 대화형·멀티모달 기능을 앱에 구현하는 **AI 모바일 서비스 개발 역량**.
    - **크로스플랫폼 모바일 개발자 (경로 C):** 한 번의 코드로 안드로이드와 iOS를 동시에 완성해 두 스토어에 한꺼번에 출시하는 개발자입니다. 남들이 두 번 만들 것을 한 번에 끝내는 효율로, 빠르고 일관된 출시를 주도하는 쾌감을 맛볼 수 있습니다. → *AI 활용 능력·역량:* AI 코딩 어시스턴트로 반복 UI·플랫폼 대응 코드를 빠르게 완성하는 **크로스플랫폼 개발 생산성 역량**.
    - **엣지 AI 연구원(대학원 진학) (경로 D):** 단말 위에서 AI가 더 빠르고 정확하게 돌게 하는 방법을 파고들어 논문으로 세상에 내놓는 연구자입니다. 아직 아무도 풀지 못한 문제에 내 이름을 남기며 기술의 최전선을 넓히는 보람을 느낄 수 있습니다. → *AI 활용 능력·역량:* 모델 경량화·엣지 추론 최적화를 실험하고 성능을 정량 비교하는 **엣지 AI 연구·최적화 역량**.

### 10-5. 상위 수준 보완 권고

> 아래는 네이버 부스트캠프 iOS/Android·KAIST·성균관대 SW 등 모바일SW·온디바이스AI 특성화 **상위 비교군** 및 산업 표준 정렬을 위한 **보완 권고**입니다. **공식 교과를 대체하지 않으며**, 2027학년도 교과 개편 시 심의 의견·향후 개선 계획으로 활용합니다.

| 보완 영역 | 반영 위치 | 추가하면 좋은 내용 | 기대 효과 |
| --- | --- | --- | --- |
| 선언형 UI 심화 | M4 | SwiftUI/Jetpack Compose 상태관리·단방향 데이터 흐름·프리뷰 기반 컴포넌트 설계, 명령형 View 대비 선언형 전환 실습 | 채용 필수 키워드인 선언형 UI 숙련도 확보, 부스트캠프 수준 UI 포트폴리오 |
| 온디바이스 AI 런타임 표준 | M5 | Core ML·LiteRT(구 TFLite)·Gemini Nano(AICore)·ExecuTorch 런타임 비교, 모델 변환·양자화(INT8/INT4) 파이프라인 핸즈온 | 단일 엔진 의존 탈피, 플랫폼별 추론 스택 이식 역량 |
| 모바일 RAG · 경량 LLM | M6 | 온디바이스 임베딩·벡터 검색, sLM(3B급)+로컬 인덱스 RAG, ML Kit GenAI/Foundation Models 도구호출 연동 | 오프라인 동작 대화형 앱 구현, KAIST 수준 LLM 응용 역량 |
| 온디바이스 프라이버시 추론 | M5, M6 | 개인정보 보호 추론(데이터 단말 비전송), 차등 프라이버시·온디바이스 처리 설계, 권한 최소화·민감정보 마스킹 | 프라이버시 강조 산업 표준 부합, 신뢰성 있는 AI 앱 설계 |
| 앱 성능·배터리 최적화 | M4, M5 | Instruments/Android Profiler 기반 프로파일링, 추론 지연·메모리·전력 측정, 백그라운드 작업·스레드 최적화 | 추론 지연 200ms·배터리 효율 정량 목표 달성, 실서비스 품질 |
| 모바일 CI/CD 자동화 | M3, M7 | Fastlane 기반 빌드·서명·스토어 배포 자동화, GitHub Actions 모바일 파이프라인, 자동 테스트·베타 배포(TestFlight) | 출시형 포트폴리오 운영 역량, 협업·배포 표준 내재화 |
