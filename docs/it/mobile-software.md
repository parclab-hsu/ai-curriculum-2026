...
!!! info "진출 직무 설명 — 이 직무는 어떤 일을 하나요?"
    ...
    - **온디바이스 AI 엔지니어 (경로 A):** 앱 안에서 서버 없이도 돌아가는 AI 기능을 만든다. 큰 모델을 작게 줄여 휴대폰·태블릿 같은 단말에서 바로 추론되게 하는 일이 중심이다. → *AI 활용:* Gemini Nano·Core ML 같은 경량 모델을 앱에 심고, 양자화로 속도와 배터리를 맞춘다.
    - **AI 모바일 서비스 개발자 (경로 B):** 챗봇·요약·이미지 설명처럼 AI를 쓴 모바일 서비스를 기획대로 만들어 스토어에 올린다. 사용자가 실제로 쓰는 화면과 AI 기능을 잇는 역할이다. → *AI 활용:* LLM API와 RAG를 연동해 대화형·멀티모달 기능을 앱에 붙인다.
    - **크로스플랫폼 모바일 개발자 (경로 C):** 안드로이드와 iOS를 한 번의 코드로 함께 만들어 두 스토어에 동시에 내는 개발자다. 앱을 빠르고 일관되게 출시하는 데 강점이 있다. → *AI 활용:* AI 코딩 어시스턴트로 반복 UI·플랫폼 대응 코드를 빠르게 작성한다.
    - **엣지 AI 연구원(대학원 진학) (경로 D):** 단말에서 AI가 더 빠르고 정확하게 돌게 하는 방법을 연구하고 논문으로 정리한다. 대학원·연구소로 이어지는 진로다. → *AI 활용:* 모델 경량화·엣지 추론 최적화를 실험하고 성능을 정량 비교한다.

### 10-5. 상위 수준 보완 권고

> 아래는 네이버 부스트캠프 iOS/Android·KAIST·성균관대 SW 등 모바일SW·온디바이스AI 특성화 **상위 비교군** 및 산업 표준 정렬을 위한 **보완 권고**다. **공식 교과를 대체하지 않으며**, 2027학년도 교과 개편 시 심의 의견·향후 개선 계획으로 활용한다.
