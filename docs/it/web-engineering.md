...
!!! info "진출 직무 설명 — 이 직무는 어떤 일을 하나요?"
    ...
    - **백엔드·클라우드 엔지니어 (경로 A):** 눈에 안 보이는 서버·데이터베이스와 클라우드 인프라를 만들고 운영한다. 많은 사용자가 몰려도 서비스가 안 멈추게 하는 일이 핵심이다. → *AI 활용:* AI 코딩 어시스턴트로 API·인프라 코드를 짜고, 배포·모니터링을 자동화한다.
    - **AI 웹 풀스택 개발자 (경로 B):** 화면(프론트)부터 서버까지 웹 전체를 만들되, 검색·챗봇 같은 AI 기능을 얹은 서비스를 완성한다. → *AI 활용:* 벡터DB·RAG와 LLM API를 붙여 대화형·검색형 AI 웹 기능을 구현한다.
    - **프론트엔드 개발자 (경로 C):** 사용자가 직접 보고 누르는 웹 화면을 만든다. 빠르고 쓰기 편한 UI를 React/Next.js로 구현하는 역할이다. → *AI 활용:* AI 코딩 어시스턴트로 컴포넌트·반복 UI 코드를 빠르게 작성하고 검증한다.
    - **AI 웹 서비스 창업·스타트업 CTO (경로 D):** 직접 서비스를 기획해 빠르게 만들어 시장에 내놓고, 기술 전반을 책임진다. 적은 인원으로 빠르게 만드는 능력이 중요하다. → *AI 활용:* 바이브 코딩으로 MVP를 신속히 만들어 실서비스로 배포·검증한다.
