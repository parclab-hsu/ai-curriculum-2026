...
전자트랙은 임베디드 시스템, 전력전자, 회로·제어, 로보틱스를 기반으로 하드웨어와 소프트웨어를 잉는 전자공학 핵심 인재를 양성하는 트랙이다. 산업 현장이 생성형 AI → 에이전트 AI → Physical AI로 이동하면서 '지능을 물리 세계에 구현하는' 엣지/임베디드 영역의 중요성이 급격히 커지고 있다.
...
- 임베디드 직무 요구 스택이 C/MCU/RTOS 전통 영역에서 **AI 가속기 연동, 멀티미디어·비전 SW**로 확장(예: 차량용 AI 가속기 개발 보스반도체 등).
...
| 구분 | 내용 |
| --- | --- |
| **핵심 직무 역량** | C/C++, MCU·펀웨어, RTOS, 디바이스 드라이버, 부트로더/커널, 회로설계, 전력변환/모터제어, 제어이론, 신호처리 |
| **AI 융합 역량** | 엣지 AI·온디바이스 AI 추론, TinyML, 모델 경량화/양자화, NPU·AI 가속기 연동, 비전·센서퓨전, Physical AI(체화 AI) 제어 |
...
- **팭리스/중견**: 보스반도체(차량용 AI 가속기 임베디드 SW), 자동차 전장·전력반도체 중견사
- **스타트업**: 레인보우로보틱스·두산로보틱스 등 로보틱스, 온디바이스 AI·엣지 디바이스 스타트업

## 6. 교육과정 개편 시사점

1. **임베디드 + 엣지 AI 트랙 신설**: 펀웨어/MCU 교과에 TinyML·온디바이스 추론·모델 경량화·NPU 연동 실습을 결합한 캡스톤.
...
- SK하이닉스 2026 채용 / LG전자 2026 채용(자소서을) / 현대모비스 로보틱스 기구/시스템
- 임베디드 개발자 채용(원티드) / 보스반도체 임베디드 SW(잡코리아)
- 전력전자/SiC 동향(THE ELEC) / 글로벌 산업 트렌드(반도체·스마트팩토리·피지캬AI) / 피지캬 AI 정리(2026)
...
| 디지털·임베디드 기반 | 학부공통 | 디지털논리, 마이크로프로세서, 임베디드 C | 임베디드 타깃에 펌웨어 설계·구현 | 디지털논리회로, 임베디드시스템 |
| 아날로그·신호처리 심화 | 트랙전공 | 아날로그회로, 센서 인터페이스, DSP | 센서 신호 취득·필터링·특징추출 구현 | 아날로그회로설계, 디지털신호처리 |
| 엣지 AI·TinyML | 트랙전공 | 경량 신경망, 양자화·프루닝, MCU 추론 | MCU/엣지보드에 모델 포팅·성능 측정 | 엣지AI, TinyML실습 |
| 지능형 전자시스템 설계 | 트랙전공 | 센서-신호-AI 통합, 저전력 설계 | 엣지 추론 파이프라인 통합 시스템 구현 | 임베디드AI설계, 전자시스템캡스톤 |

### 10-2. 모듈 간 관계 (트랙·학부·단과대학)

- **위계**: 단과대학 공통(AI·데이터 기초) → 전기전자공학부 공통(회로·전자기·신호 / 디지털·임베디드) → 전자트랙 전공심화(아날로그·DSP → 엣지AI·TinyML → 지능형 전자시스템 설계).
