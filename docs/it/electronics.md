...
!!! info "진출 직무 설명 — 이 직무는 어떤 일을 하나요?"
    ...
    - **엣지 AI 임베디드 엔지니어 (경로 A):** 마이크로컨트롤러·소형 보드 같은 작은 기기 안에 AI를 넣어 서버 없이 바로 판단하게 만든다. 센서 신호를 받아 기기가 스스로 처리하도록 한다. → *AI 활용:* TinyML·경량 모델을 MCU/NPU에 올려 저전력으로 실시간 추론한다.
    - **센서·신호처리 시스템 엔지니어 (경로 B):** 센서에서 들어온 잡음 섞인 신호를 깨끗하게 다듬고, 필요한 정보만 뽑아내는 시스템을 설계한다. → *AI 활용:* 신호에서 특징을 뽑아 AI 진단·이상 감지 모델에 연결한다.
    - **Physical AI 로보틱스 제어 엔지니어 (경로 C):** 로봇이 주변을 인식하고 스스로 움직이도록 인지-판단-구동을 잇는 제어 시스템을 만든다. → *AI 활용:* ROS2 위에서 강화·모방학습과 엣지 추론으로 로봇의 인지·구동을 통합한다.
    - **EV 전력전자·모터제어 엔지니어 (경로 D):** 전기차의 인버터·모터를 효율적이고 안전하게 돌리는 전력 회로와 제어기를 설계한다. → *AI 활용:* AI로 회로 상태를 감시하고 열관리·모터제어를 최적화한다.
