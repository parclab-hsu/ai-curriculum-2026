...
!!! info "진출 직무 설명 — 이 직무는 어떤 일을 하나요?"
    ...
    - **컴퓨터비전·Physical AI 엔지니어 (경로 A):** 카메라·센서로 들어온 영상을 컴퓨터가 알아보게 만들고, 그 인식 결과로 로봇이나 기기를 움직이게 한다. 현실 세계를 이해하는 AI를 다룬다. → *AI 활용:* 객체 인식·자세 추정·SLAM 모델을 실시간 인식·제어에 연결한다.
    - **XR·생성형 콘텐츠 개발자 (경로 B):** AR/VR 같은 몰입형 콘텐츠를 게임엔진으로 만든다. 3D 에셋이나 디지털휴먼을 붙여 체험형 서비스를 완성하는 역할이다. → *AI 활용:* 텍스트-3D·이미지 생성으로 에셋을 만들고 AI NPC·디지털휴먼을 콘텐츠에 넣는다.
    - **버추얼 프로덕션 테크니컬 디렉터 (경로 C):** LED월·실시간 렌더링으로 촬영·방송 현장을 가상 배경과 합쳐 만든다. 제작 파이프라인 전체를 기술적으로 총괄한다. → *AI 활용:* 생성형 3D·디지털휴먼 도구로 배경·캐릭터 제작을 자동화하고 품질을 높인다.
    - **디지털트윈·로보틱스 시뮬레이션 엔지니어 (경로 D):** 공장·로봇을 가상 공간에 똑같이 만들어 미리 시험하고, AI 학습용 데이터를 시뮬레이션으로 만들어 낸다. → *AI 활용:* Isaac Sim 등으로 합성데이터를 생성하고 Sim-to-Real로 로봇 학습을 돕는다.

### 10-5. 상위 수준 보완 권고

> 아래는 KAIST 문화기술대학원·한국예술종합학교·중앙대 첨단영상대학원 등 XR·디지털콘텐츠 특성화 **상위 비교군** 및 산업 표준 정렬을 위한 **보완 권고**다. **공식 교과를 대체하지 않으며**, 2027학년도 교과 개편 시 심의 의견·향후 개선 계획으로 활용한다.
