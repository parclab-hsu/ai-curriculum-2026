...
> 한성대학교 IT공과대학 컴퓨터공학부 · 2026학년도 AI융합 교육과정 개편 리서치 (조사일: 2026-06-25) / 구 디지털콘텐츠·가상현실트랙
...
!!! abstract "이 트랙과 향후 10년 고용"
    ...
    - **미국(BLS)·글로벌(WEF):** WEF는 소프트웨어 개발과 AI/ML을 성장 직무로 분류하고 생성형 AI 확산으로 기업 86%가 전환을 예고했으며, 미국 컴퓨터·수학 직군도 +10.1% 성장한다 — 실시간 그래픽스·디지털휴먼 제작 역량과 직결된다.
...
1. **실시간 3D 엔진 + AI 융합 단일 트랙화.** 모델링·렌더링 기초 위에 생성형 3D(Diffusion·Gaussian Splatting/NeRF) 실습을 결합 → 'AI로 에셋 생성 후 엔진에서 실시간 구현'하는 end-to-end 역량.
...
| 그래픽스 수학·기반 | 학부공통 | 선형대수, 기하, 컴퓨터그래픽스 원리 | 3D 변환·렌더링 원리 이해 및 구현 | 선형대수, 컴퓨터그래픽스 |
| XR·게임엔진 개발 | 트랙전공 | Unity/Unreal, AR/VR, 인터랙션 디자인 | 인터랙티브 XR 콘텐츠 독립 구현 | 게임엔진프로그래밍, XR콘텐츠개발 |
| 컴퓨터비전·인식 | 트랙전공 | 객체인식, 자세추정, SLAM, 딥러닝 비전 | 실시간 비전 인식 시스템 구현 | 컴퓨터비전, 영상처리 |
| Physical AI·로보틱스 | 트랙전공 | 센서융합, 로봇제어, 시뮬레이션, 강화학습 | 현실 연동 지능 제어 시스템 구현 | 피지컬AI, 로보틱스시뮬레이션 |
| 생성형 AI 콘텐츠 | 트랙전공 | 텍스트-3D, 이미지·영상 생성, 디지털 휴먼 | 생성형 AI 기반 콘텐츠 파이프라인 구축 | 생성형AI콘텐츠, 디지털휴먼 |
| 실감미디어 캡스톤 | 트랙전공 | 기획·제작·시연, 산학 협업 | 몰입형 XR·AI 콘텐츠 종합 완성 | 캡스톤디자인 |
