...
!!! info "진출 직무 설명 — 이 직무는 어떤 일을 하나요?"
    ...
    - **스마트제조 엔지니어 (경로 A):** 공장의 생산 공정과 설비를 데이터로 살펴보고 더 잘 돌아가도록 개선하는 일이다. 설비 고장을 미리 막고 생산 흐름의 병목을 줄인다. → *AI 활용:* 공정·설비 데이터를 학습해 고장을 예측하고(예측정비) 이상 징후를 자동으로 잡아낸다.
    - **SCM·수요예측 플래너 (경로 B):** 얼마나 팔릴지 예측해 원자재 구매·재고·물류 계획을 세우는 일이다. 공급망 전체가 부족하지도 남지도 않게 균형을 맞춘다. → *AI 활용:* 과거 판매·물류 데이터로 수요를 예측해 재고와 배송 계획을 더 정확하게 잡는다.
    - **산업공학 컨설턴트(운영혁신·OE) (경로 C):** 회사의 일하는 방식과 공정·물류 흐름을 진단해 낭비를 줄이고 더 효율적으로 바꾸는 일이다. → *AI 활용:* 시뮬레이션·디지털트윈으로 여러 개선안을 가상으로 검증한 뒤 가장 나은 방법을 제안한다.
    - **품질엔지니어(AI 비전검사) (경로 D):** 제품에 불량이 없는지 검사하고 품질을 일정하게 유지·관리하는 일이다. 불량 원인을 찾아 재발을 막는다. → *AI 활용:* AI 영상 검사로 불량을 자동 판별하고, 통계 기법(SPC)과 결합해 품질 문제를 미리 예측한다.
