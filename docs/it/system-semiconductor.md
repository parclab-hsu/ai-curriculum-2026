...
시스템반도체트랙은 반도체 설계(RTL/회로)·검증·소자·공정·패키징을 아우르는 비메모리 중심 인재를 양성하는 트랙입니다. AI 반도체(NPU)·HBM·팹리스·파운드리·첨단패키징으로 산업 축이 이동하면서, 설계·검증 역량에 AI/머신러닝 이해를 결합한 인재 수요가 급증하고 있습니다.
...
> 참고: 신입 '설계' 직무는 실제로 **설계와 검증**이 묶인 직무이며, 칩 복잡도 증가로 "설계 30% : 검증 70%"라 불릴 만큼 SystemVerilog/UVM 기반 검증 역량 비중이 높습니다.
...
> **학문 분야 정체성**: 시스템반도체트랙은 반도체 소자·디지털설계·SoC를 기반으로 AI 연산을 가속하는 하드웨어를 설계·검증하는 시스템반도체 전문 인재를 양성합니다.

반도체공학의 정체성(소자·회로·디지털설계·SoC)을 유지하면서, AI 워크로드를 위한 NPU/가속기 설계와 AI 기반 EDA 설계 자동화를 결합합니다. 구체적·측정 가능한 교육 목표는 다음과 같습니다.
...
!!! info "진출 직무 — 이런 전문가로 성장할 수 있어요"
    ...
    - **AI 반도체(NPU) 설계 엔지니어 (경로 A):** AI 시대의 심장인 신경망 가속기(NPU)를, 어떤 연산을 회로로 새길지부터 직접 결정하며 만들어 내는 사람입니다. 손톱만 한 칩 위에 성능·전력·면적을 저울질해 그려 넣은 내 설계가 수백만 대의 기기에 실려 나가는 스케일을 경험할 수 있습니다. → *AI 활용 능력·역량:* 신경망 연산 패턴을 분석해 하드웨어 가속 구조를 결정하는 **AI 가속기 아키텍처 설계 역량**.
    - **SoC·EDA 설계 자동화 엔지니어 (경로 B):** 수십억 개의 트랜지스터를 하나의 칩에 담는 SoC를, 반복 작업을 AI 자동화로 밀어내며 설계하는 사람입니다. 사람이 몇 주 걸리던 일을 도구로 하루 만에 끝내며 설계 생산성 자체를 바꾸는 쾌감이 있습니다. → *AI 활용 능력·역량:* AI 보조 EDA로 설계 코드 생성·최적화·오류 점검을 자동화하는 **AI 기반 설계 자동화 역량**.
    - **디지털 검증(DV) 엔지니어 (경로 C):** 수백억이 든 칩이 세상에 나가기 전, "이 칩은 절대 실패하지 않는다"를 증명하는 마지막 관문을 지키는 사람입니다. 내가 짠 시나리오가 숨어 있던 치명적 버그를 잡아내는 순간의 몰입과 책임감이 매력입니다. → *AI 활용 능력·역량:* AI로 테스트 케이스를 자동 생성하고 검증 사각지대를 찾아 커버리지를 끌어올리는 **AI 검증 자동화 역량**.
    - **아날로그·혼성신호 IC 설계 엔지니어 (경로 D):** 소리·전압처럼 끊임없이 변하는 실세계 신호를 디지털과 잇는, 소수만 다룰 수 있는 '장인의 영역'을 맡는 사람입니다. 센서·전원·통신 칩의 심장부를 직접 그리며 대체 불가능한 전문가로 자리 잡는 성취를 얻습니다. → *AI 활용 능력·역량:* AI 기반 EDA로 회로 특성 최적화와 설계 검토를 가속하는 **아날로그·혼성신호 설계 최적화 역량**.

### 10-5. 상위 수준 보완 권고

> 아래는 KAIST·서울대 반도체공동연구소·성균관대 반도체시스템공학 등 시스템반도체·AI 반도체 특성화 **상위 비교군** 및 산업 표준 정렬을 위한 **보완 권고**입니다. **공식 교과를 대체하지 않으며**, 2027학년도 교과 개편 시 심의 의견·향후 개선 계획으로 활용합니다.
