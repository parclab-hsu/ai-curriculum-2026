...
시스템반도체트랙은 반도체 설계(RTL/회로)·검증·소자·공정·패키징을 아우르는 비메모리 중심 인재를 양성하는 트랙이다. AI 반도체(NPU)·HBM·팹리스·파운드리·첨단패키징으로 산업 축이 이동하면서, 설계·검증 역량에 AI/머신러닝 이해를 결합한 인재 수요가 급증.
...
> 참고: 신입 '설계' 직무는 실제로 **설계 + 검증**이 묶인 포지션이며, 칩 복잡도 증가로 "설계 30% : 검증 70%"라 불릴 만큼 SystemVerilog/UVM 기반 검증 역량 비중이 높다.
...
- K-팹리스 양산 경쟁(시사저널e) / AI 반도체 스타트업 인재 경쟁(한국경제)
