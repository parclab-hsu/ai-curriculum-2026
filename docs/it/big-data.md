...
| --- | --- | --- | --- | --- |
| AI·데이터 공통 기초 | 단과대학공통 | Python·데이터 처리, 생성형 AI/LLM 활용, AI 코딩 어시스턴트, AI 윤리 | AI 도구로 데이터 처리·프로토타이핑 수행 | 컴퓨터프로그래밍, 프로그래밍랩, AI·데이터 리터러시(제안) |
| 컴퓨터과학 기반 | 학부공통 | 자료구조, 알고리즘, 운영체제, 네트워크 | 시스템 동작 원리 이해 및 효율적 알고리즘 설계 | 자료구조, 알고리즘, 데이터통신, 운영체제(제안) |
| 데이터 기반 수학·통계 | 학부공통 | 확률·통계, 선형대수, 데이터 시각화 | 데이터 기반 의사결정·모델링 수행 | 확률및통계, 선형대수, 데이터시각화(제안) |
| 데이터 엔지니어링 | 트랙전공 | 데이터베이스, 분산처리(Spark), ETL, 데이터레이크 | 대용량 데이터 파이프라인 구현 | 데이터베이스, 빅데이터기초 |
| 머신러닝·딥러닝 | 트랙전공 | 지도/비지도학습, 딥러닝, 모델 평가 | 데이터 기반 예측·분류 모델 구현 | 데이터마이닝, 머신러닝(제안), 딥러닝(제안) |
| MLOps·모델 운영 | 트랙전공 | CI/CD, 실험추적, 배포·모니터링, 재학습 | 재현 가능한 ML 시스템 운영 | MLOps(보완 제안), 데이터플랫폼(제안) |
| RAG·LLM 데이터 활용 | 트랙전공 | 벡터DB, RAG, LLM 분석·질의 | 비정형 데이터 LLM 활용 시스템 구현 | 생성형AI데이터분석(보완 제안/RAG·VectorDB), RAG시스템(보완 제안) |
| 빅데이터 캡스톤 | 트랙전공 | 데이터·AI 파이프라인 구축·운영, 산학 협업 | 데이터 AI 시스템 종합 완성 | 캡스톤디자인(제안) |
