...
!!! info "진출 직무 설명 — 이 직무는 어떤 일을 하나요?"
    ...
    - **MLOps 엔지니어 (경로 A):** 머신러닝 모델을 한 번 만들고 끝내는 게 아니라, **자동으로 학습·배포·감시되도록 파이프라인을 구축·운영**하는 엔지니어입니다(개발과 운영의 연결). → *AI 활용:* 모델 재학습·버전관리·모니터링을 자동화해 서비스에 안정적으로 올립니다.
    - **AI 데이터 분석가 (경로 B):** 데이터에서 **의미 있는 패턴·인사이트를 찾아 의사결정을 돕는** 분석가입니다. → *AI 활용:* 생성형 AI·LLM으로 데이터를 요약·질의(RAG)하고 분석 리포트를 빠르게 만듭니다.
    - **데이터 플랫폼 엔지니어 (경로 C):** 대용량 데이터를 **모으고 저장·처리하는 시스템(데이터 파이프라인·레이크하우스)**을 설계·운영하는 엔지니어입니다. → *AI 활용:* 실시간 스트리밍·클라우드 위에 AI가 바로 쓸 수 있는 데이터 기반을 만듭니다.
    - **데이터 사이언티스트 (경로 D · 연구·대학원):** 통계·머신러닝으로 **문제를 모델로 풀고 새로운 방법을 연구**하는 인력으로, 대학원 진학이 일반적입니다. → *AI 활용:* 딥러닝·멀티모달·생성형 AI 모델을 직접 설계·실험합니다.
