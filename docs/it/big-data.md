...
빅데이터트랙은 대용량 데이터 수집·처리·분석 파이프라인과 머신러닝/MLOps, 그리고 LLM·RAG 기반 AI 서비스의 데이터 기반(backbone)을 구축하는 트랙입니다.

**AI 융합 개편 방향**: 거시 지표상 국내 기업의 **69%(2026)가 채용 시 AI 역량을 고려**합니다. 기존 데이터엔지니어링(Spark·Airflow·dbt·SQL·클라우드)에 LLM 파인튜닝·RAG·벡터DB·에이전트 워크플로우를 결합합니다.
...
1. **AI 융합형 데이터 엔지니어링 강화 권장.** Spark/Airflow/dbt/SQL/클라우드 위에 **LLM 파인튜닝·RAG·벡터DB·LangGraph/Agentic AI** 모듈을 정규 편성하는 방안을 권장.
...
> 학문 분야 정체성: 빅데이터트랙은 대규모 데이터의 수집·저장·처리·분석 SW공학 역량에 MLOps와 RAG·LLM 기반 데이터 활용을 결합하여, 데이터로부터 가치를 창출하고 신뢰성 있게 운영하는 데이터 엔지니어·ML 엔지니어를 양성합니다.
...
!!! info "진출 직무 — 이런 전문가로 성장할 수 있어요"
    ...
    - **MLOps 엔지니어 (경로 A):** 애써 만든 AI 모델이 실험실에만 머물지 않도록, 자동으로 학습·배포·감시되는 파이프라인을 지어 실서비스로 올리는 엔지니어입니다. 개발과 운영을 잇는 핵심 축으로서 "모델이 24시간 알아서 돌아간다"는 시스템을 내 손으로 완성하는 뿌듯함을 경험할 수 있습니다. → *AI 활용 능력·역량:* 모델 재학습·버전관리·모니터링을 자동화해 안정적으로 서비스에 올리는 **ML 파이프라인 운영·자동화 역량**.
    - **AI 데이터 분석가 (경로 B):** 흩어진 데이터 속에서 숨은 패턴과 인사이트를 찾아내 조직의 의사결정을 이끄는 분석가입니다. 내가 발견한 한 줄의 인사이트가 회사의 방향을 바꾸는, 데이터로 사람들을 설득하는 짜릿함을 맛볼 수 있습니다. → *AI 활용 능력·역량:* 생성형 AI·LLM으로 데이터를 요약·질의(RAG)하고 분석 리포트를 빠르게 만드는 **AI 기반 데이터 분석 역량**.
    - **데이터 플랫폼 엔지니어 (경로 C):** 대용량 데이터를 모으고 저장·처리하는 시스템(데이터 파이프라인·레이크하우스)을 직접 설계·운영하는 엔지니어입니다. 회사의 모든 데이터가 흐르는 거대한 기반을 내가 짓는다는 스케일 큰 성취를 느낄 수 있습니다. → *AI 활용 능력·역량:* 실시간 스트리밍·클라우드 위에 AI가 바로 쓸 수 있는 데이터 기반을 구축하는 **데이터 플랫폼 설계·운영 역량**.
    - **데이터 사이언티스트 (경로 D · 연구·대학원):** 통계·머신러닝으로 세상의 문제를 모델로 풀고 새로운 방법을 연구하는 인재로, 대학원 진학으로 이어지는 길입니다. 남들이 못 푼 문제에 나만의 모델로 답을 내며 기술의 최전선에 서는 자부심을 가질 수 있습니다. → *AI 활용 능력·역량:* 딥러닝·멀티모달·생성형 AI 모델을 직접 설계·실험하는 **AI 모델링·연구 역량**.

### 10-5. 상위 수준 보완 권고

> 아래는 KAIST·서울대 데이터사이언스대학원·성균관대 등 빅데이터·데이터엔지니어링 특성화 **상위 비교군** 및 산업 표준 정렬을 위한 **보완 권고**입니다. **공식 교과를 대체하지 않으며**, 2027학년도 교과 개편 시 심의 의견·향후 개선 계획으로 활용합니다.
