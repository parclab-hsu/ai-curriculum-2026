...
| --- | --- | --- | --- | --- |
| AI 핵심·에이전트 기초 | 단과대학공통 | AI/ML 기초, 생성형 AI·RAG·에이전트 기초, MLOps 입문, AI 윤리·거버넌스 | AI 핵심 개념을 이해하고 에이전트형 AI의 동작 원리를 설명·시연할 수 있다 | 인공지능 개론, 생성형AI와에이전트입문(신설 제안), AI윤리와거버넌스(제안) |
| 데이터·AI 엔지니어링 | 학부공통 | 데이터 수집·전처리·파이프라인, 데이터베이스, 특징공학 | 실데이터를 분석 가능한 형태로 가공하고 파이프라인을 구축할 수 있다 | 데이터엔지니어링(제안), 데이터베이스(제안), 데이터시각화(제안) |
| 머신러닝·딥러닝 응용 | 학부공통 | 지도·비지도학습, CNN·RNN·Transformer 활용 | 문제 유형에 맞는 모델을 선택·학습·평가할 수 있다 | 머신러닝, 딥러닝, 컴퓨터비전, 강화학습 |
| 생성형 AI·LLM 솔루션 | 학과전공 | LLM API, 프롬프트 엔지니어링, RAG, 멀티모달 응용 | 도메인 특화 생성형 AI 서비스를 설계·구현할 수 있다 | LLM응용프로그래밍(신설 제안), RAG시스템설계(신설 제안), 멀티모달AI(신설 제안) |
| AI 에이전트·자동화 | 학과전공 | 에이전트 설계, 도구 호출·오케스트레이션, 워크플로우 자동화 | 도구를 사용하는 자율 에이전트 기반 자동화 솔루션을 구축할 수 있다 | AI에이전트설계(신설 제안), 워크플로우자동화(신설 제안), 프롬프트·툴오케스트레이션(신설 제안) |
| MLOps·AI 서비스 운영 | 학과전공 | 모델 배포, CI/CD, 모니터링·재학습, 클라우드 | 모델을 서비스로 배포하고 운영·관리할 수 있다 | MLOps(신설 제안), 클라우드AI서비스(신설 제안), AI시스템운영(신설 제안) |
| 도메인 AI 응용 캡스톤 | 학과전공 | 산업 도메인 문제 정의·솔루션 통합 | 실문제를 해결하는 AI 솔루션을 완성하고 검증할 수 있다 | AI프리캡스톤디자인, 산학캡스톤디자인(제안), AI응용프로젝트(제안) |
