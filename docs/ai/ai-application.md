...
**AI 융합 개편 방향**: 생성형 AI → 에이전트 AI → Physical AI 패러다임에 맞춰 커리큘럼을 재구성한다. 거시 지표상 국내 AI 역량 요구 기업 비중이 2026년 69%(증가세가 가장 가파름)에 이르러, "AI를 만드는 사람"보다 **"AI를 잘 쓰는 사람(AI 활용·응용)"** 수요가 빠르게 늘고 있다. 학과 포지셔닝을 *모델 개발자*가 아닌 **AI 애플리케이션 엔지니어 / AI 솔루션 컨설턴트 / AI 서비스 기획자**로 설정한다.
...
!!! info "진출 직무 설명 — 이 직무는 어떤 일을 하나요?"
    ...
    - **생성형 AI 솔루션 개발자 (경로 A):** 공개된 대형 언어모델(LLM)을 가져다 챗봇·문서요약·검색 도우미 같은 서비스를 만드는 사람이다. 모델을 처음부터 만들기보다 이미 있는 모델을 잘 조합해 제품으로 완성하는 데 가깝다. → *AI 활용:* LLM에 회사 자료를 연결하는 RAG·프롬프트·에이전트를 직접 설계해 서비스로 붙인다.
    - **ML/MLOps 엔지니어 (경로 B):** 만들어진 AI 모델이 실제 서비스에서 끊기지 않고 잘 돌아가도록 배포하고 감시하며 다시 학습시키는 일을 한다. AI의 '운영·관리' 담당에 가깝다. → *AI 활용:* 모델을 컨테이너·클라우드에 올려 서빙하고, 성능이 떨어지면 자동으로 재학습·모니터링한다.
    - **도메인 AI 솔루션 컨설턴트 (경로 C):** 헬스·금융처럼 특정 분야의 현장 문제를 듣고, 어떤 AI를 어떻게 쓰면 좋을지 설계·제안하는 사람이다. 기술과 현업을 잇는 다리 역할을 한다. → *AI 활용:* 데이터를 분석해 문제를 진단하고, LLM·RAG 기반 해법을 기획·검증해 도입을 돕는다.
    - **멀티모달·컴퓨터비전 AI 엔지니어 (경로 D):** 글자뿐 아니라 이미지·영상·음성까지 함께 이해하는 AI 서비스를 만드는 사람이다. 사진 속 물체를 알아보거나 영상을 설명하는 기능을 다룬다. → *AI 활용:* 컴퓨터비전·시각언어모델(VLM)로 이미지·영상 인식 서비스를 개발·배포한다.
