...
!!! info "진출 직무 설명 — 이 직무는 어떤 일을 하나요?"
    ...
    - **자율주행 인지 SW 엔지니어 (경로 A):** 카메라·라이다·레이더 센서 데이터로 차량 주변의 사람·차·차선을 **인식(인지)하는 소프트웨어**를 만드는 엔지니어입니다. → *AI 활용:* 딥러닝 객체 인식·센서퓨전 모델을 학습·최적화해 "무엇이 어디 있는지"를 실시간으로 판단합니다.
    - **차량제어·SDV 임베디드 엔지니어 (경로 B):** 인지 결과를 받아 **차량을 실제로 조향·가감속하는 제어 소프트웨어**와, 이를 차량 컴퓨터(ECU)에 올리는 임베디드 시스템을 개발합니다. SDV(소프트웨어 중심 차량)의 핵심 직무입니다. → *AI 활용:* 규칙 기반 제어에 학습 기반 제어를 결합하고, 실시간·안전 요건을 만족하도록 최적화합니다.
    - **모빌리티 데이터·AI 엔지니어 (경로 C):** 주행 로그·센서 데이터를 모아 **AI 모델을 학습·배포(MLOps)**하고 성능을 개선하는 엔지니어입니다. → *AI 활용:* 대규모 주행 데이터 파이프라인을 만들고, Edge AI로 차량 안에서 바로 추론이 돌아가게 합니다.
    - **자율주행/로보틱스 연구원 (경로 D · 대학원):** 자율주행·로봇의 **인지·판단·제어 알고리즘을 연구**하는 R&D 인력으로, 대학원 진학이 일반적입니다. → *AI 활용:* 강화·모방학습, Sim-to-Real, Physical AI 실시간 제어 등 최신 기법을 연구합니다.
