...
미래모빌리티학과는 자율주행·전동화(EV)·SDV(소프트웨어 정의 차량)·UAM 등 차세대 이동수단의 **설계·제어·소프트웨어**를 다루는 학과입니다.

**AI 융합 개편 방향:** 자동차 산업의 무게중심이 하드웨어에서 **소프트웨어·AI**로 이동하면서, 기존 기계·전자 중심 교육을 **인지(perception)→판단(planning)→제어(control) 풀스택 + 딥러닝·MLOps·Edge AI** 중심으로 재편합니다. PARC Lab의 실시간 제어·모빌리티 역량과 정렬해 졸업생이 Physical AI 기반 자율 이동체 개발에 즉시 투입 가능하도록 합니다.
...
> 학문 분야 정체성: 미래모빌리티학과는 **공학·역학 기초** 위에 자율주행·전동화(EV)·SDV(소프트웨어 정의 차량)를 **인지→제어→AI/Physical AI**로 구현하는 차세대 이동체 인재를 양성합니다.

아래 목표는 **한성대 미래모빌리티학과 공식 교과목**(9-1)에 근거해 측정 가능하게 설정합니다. 생성형 AI·MLOps·Edge AI 등 심화 항목은 단과대학 공통 또는 보완(제안) 모듈로 운영합니다.
...
!!! info "출처·기준"
    한성대학교 미래모빌리티학과 **공식 교육과정**([hansung.ac.kr/CreCon/2798](https://www.hansung.ac.kr/CreCon/2798/subview.do)) 기준. 아래 9-2 이수 흐름과 10장 모듈·학습경로는 이 **공식 교과목을 역량 중심으로 재구성**한 것이며, 공식 목록에 없는 보완 제안 과목은 **(제안)**으로 표기합니다.
...
> 출처: 한성대학교 미래모빌리티학과 「모듈형 전공교육과정 안내 및 학습자 진로 가이드」(공식 교과목 기준, 2026-06-30 개정). 아래 표는 공식 가이드의 모듈 안내 표를 그대로 옮긴 것으로, **모든 구성 교과목은 공식 교과목**입니다(이수구분: 전기=전공기초, 전필=전공필수, 전선=전공선택).
...
공식 가이드의 로드맵 표를 그대로 옮긴 것으로, 각 모듈 교과목이 **언제 개설되는지**(학년-학기)를 한눈에 보여줍니다.
...
> 아래 경로의 과목은 모두 **공식 교과목명** 기준입니다(보완 항목만 (제안) 표기).
...
!!! info "진출 직무 — 이런 전문가로 성장할 수 있어요"
    ...
    - **자율주행 인지 SW 엔지니어 (경로 A):** 카메라·라이다·레이더가 보내는 신호를 읽어 차가 스스로 세상을 '보게' 만드는 자율주행의 눈을 설계하는 엔지니어입니다. 내가 만든 소프트웨어가 실제 도로에서 사람과 차를 알아보고 안전을 지키는, SF가 현실이 되는 순간을 직접 만들어 냅니다. → *AI 활용 능력·역량:* 딥러닝 객체 인식·센서퓨전 모델을 학습·최적화해 "무엇이 어디 있는지"를 실시간으로 판단하는 **자율주행 인지 SW 개발 역량**.
    - **차량제어·SDV 임베디드 엔지니어 (경로 B):** 인지 결과를 받아 차를 실제로 돌리고 멈추게 하는 제어 소프트웨어를 짜고 차량 컴퓨터에 올리는, 소프트웨어 중심 차량(SDV)의 심장을 다루는 직무입니다. 내 코드 한 줄이 수 톤의 차를 정밀하게 움직인다는 짜릿한 책임감을 경험할 수 있습니다. → *AI 활용 능력·역량:* 규칙 기반 제어에 학습 기반 제어를 결합하고 실시간·안전 요건을 만족하도록 최적화하는 **차량제어·SDV 임베디드 개발 역량**.
    - **모빌리티 데이터·AI 엔지니어 (경로 C):** 수많은 차가 쏟아 내는 주행 데이터를 모아 자율주행을 점점 더 똑똑하게 키워 내는 엔지니어입니다. 어제보다 오늘 더 잘 달리는 AI를 내 손으로 성장시키는 성취를 매일 확인할 수 있습니다. → *AI 활용 능력·역량:* 대규모 주행 데이터 파이프라인을 구축하고 Edge AI로 차량 안에서 바로 추론이 돌게 하는 **모빌리티 데이터·MLOps 역량**.
    - **자율주행/로보틱스 연구원 (경로 D · 대학원):** 아직 아무도 풀지 못한 자율주행·로봇의 인지·판단·제어 난제에 도전하는 R&D 인력으로, 세상을 앞서가는 기술을 직접 만들어 냅니다. 내 연구가 논문과 특허가 되어 미래 모빌리티의 방향을 바꾸는 보람이 매력입니다. → *AI 활용 능력·역량:* 강화·모방학습, Sim-to-Real, Physical AI 실시간 제어 등 최신 기법을 연구하는 **자율주행 AI 연구 역량**.


### 10-5. 상위 수준 보완 권고

> 아래는 국민대·한양대·아주대·서울과기대 등 미래자동차·모빌리티 특성화 **상위 비교군** 및 산업 표준 정렬을 위한 **보완 권고**입니다. **공식 교과를 대체하지 않으며**, 2027학년도 교과 개편 시 심의 의견·향후 개선 계획으로 활용합니다.
