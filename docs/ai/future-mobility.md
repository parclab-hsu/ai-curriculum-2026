...
> 본 모듈은 **한성대 공식 교과과정([CreCon/2798](https://www.hansung.ac.kr/CreCon/2798/subview.do))**을 기본 데이터로 3~4과목 단위로 재구성했다. 공식 목록에 없는 과목은 **(예시)**로 표기. 확인일 2026-06-28.

| 모듈명 | 계층 | 핵심 역량·주제 | 학습 성과 | 대표 교과(공식/예시) |
| --- | --- | --- | --- | --- |
| AI 핵심·머신러닝 | 단과대학 공통 | AI/ML 기초, 생성형 AI·에이전트·AI 윤리 | ML 모델 구현, AI 윤리 이슈 분석 | 인공지능 및 머신러닝개론 · 인공지능 및 머신러닝 실습 · 생성형AI·에이전트(예시) |
| 모빌리티 공학·수학 기초 | 학부/학과 공통 | 4대 역학·공학수학·공학물리 | 역학 해석, 수학적 모델링 | 미래모빌리티개론 · 모빌리티 공학수학 · 기초역학Ⅰ · 기초역학Ⅱ |
| 전자·컴퓨팅·프로그래밍 기초 | 학부 공통 | C 프로그래밍, 전기전자회로, 논리회로 | 임베디드·AI 코드 작성, 회로·구조 해석 | C프로그래밍 · 모빌리티 프로그래밍 실습 · 모빌리티 전기전자회로 · 논리회로 및 컴퓨터구조 |
| 자율주행 인지·센서 | 학과 전공 | 센서·모터제어, 센서 응용·융합, 자율주행 인지 | 센서 처리·인지 파이프라인 구현 | 센서처리와 모터제어 · 모빌리티 센서 응용설계 · 자율주행시스템 응용 |
| 제어·임베디드·SDV | 학과 전공 | 제어설계·제어공학, 임베디드·OS, ECU | 제어 알고리즘·ECU·임베디드 통합 | 모빌리티 제어설계 · 모빌리티 제어공학 · 임베디드시스템 및 운영체제 · 모빌리티 전자제어유닛 실습 |
| Physical AI·로보틱스·설계 | 학과 전공 | 시뮬레이션, 이동로봇·ROS, 기계설계(CAD) | 시뮬-실차 연계, 로봇 ROS·기구 설계 구현 | Physical AI & 로보틱스 시뮬레이션 · 이동로봇과 ROS · 기계제품설계(CAD) · 드론 및 UAM |
| 모빌리티 종합설계 캡스톤 | 학과 전공 | 인지→판단→제어 통합, 실증·PBL | 자율주행/SDV 풀스택 프로토타입 완성·발표 | 모빌리티 종합설계 프로젝트 · 스마트모빌리티실증랩 · 모빌리티 창의설계 |

#### 10-1 다이어그램 — 1~4학년 모듈 로드맵

학년이 올라갈수록 기초 → 전공심화 → 통합으로 모듈이 연결되는 전체 구조다(공식 교과 이수 학년 기준).

```mermaid
flowchart LR
  subgraph Y1["1학년 · 입문"]
    A0["미래모빌리티개론<br/>모빌리티 창의설계 · C프로그래밍"]
  end
  subgraph Y2["2학년 · 전공기반"]
    M1["① AI 핵심·머신러닝<br/>인공지능 및 머신러닝개론"]
    M2["② 공학·수학 기초<br/>모빌리티 공학수학 · 기초역학Ⅰ · 공학물리"]
    M3["③ 전자·컴퓨팅·프로그래밍<br/>전기전자회로 · 논리회로 · 자료구조"]
  end
  subgraph Y3["3학년 · 전공심화"]
    M4["④ 자율주행 인지·센서<br/>센서처리와 모터제어 · 센서 응용설계 · 자율주행시스템 응용"]
    M5["⑤ 제어·임베디드·SDV<br/>모빌리티 제어공학 · 임베디드시스템 · 전자제어유닛 실습"]
    M6["⑥ Physical AI·로보틱스·설계<br/>머신러닝 실습 · 로보틱스 시뮬레이션 · 기계제품설계"]
  end
  subgraph Y4["4학년 · 통합"]
    CAP["⑦ 종합설계 캡스톤<br/>모빌리티 종합설계 프로젝트 · 스마트모빌리티실증랩 · 이동로봇과 ROS"]
  end
  A0 --> M1 --> M6 --> CAP
  A0 --> M2 --> M4 --> CAP
  A0 --> M3 --> M5 --> CAP
...
각 모듈이 기르는 핵심 역량을, 기업이 채용에서 요구하는 역량(4장 요구 직무 역량 기준)과 연결한다.

| 모듈 | 핵심 역량(학습) | 매핑되는 기업 요구 역량 |
| --- | --- | --- |
| ① AI 핵심·머신러닝 | AI/ML 기초·AI 윤리 | 딥러닝(PyTorch/TensorFlow) 모델 개발·튜닝 |
| ② 공학·수학 기초 | 4대 역학·수학적 모델링 | 차량 동역학 이해, 제어 알고리즘 기반 |
| ③ 전자·컴퓨팅·프로그래밍 | C·전기전자·논리회로 | C/C++·Python 임베디드·AI SW, 회로 이해 |
| ④ 자율주행 인지·센서 | 센서처리·자율주행 인지 | 객체검출·차선인식, 다중센서 캘리브레이션·센서퓨전 |
| ⑤ 제어·임베디드·SDV | 제어·ECU·임베디드 통합 | 차량제어 SW, AUTOSAR·CAN/Ethernet, 실차 검증 |
| ⑥ Physical AI·로보틱스·설계 | ML·시뮬레이션·ROS·설계 | MLOps·Edge AI(Jetson)·VLA, Sim-to-Real |
| ⑦ 종합설계 캡스톤 | 인지→제어 통합 | 자율주행/SDV 풀스택 개발·테스트 |
