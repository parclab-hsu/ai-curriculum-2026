...
!!! info "진출 직무 설명 — 이 직무는 어떤 일을 하나요?"
    ...
    - **AI 콘텐츠 기획 PD (경로 A):** 웹툰·영상 같은 콘텐츠의 기획부터 제작·편집까지 전체 과정을 이끄는 사람이다. 이야기를 짜고 어떤 그림·영상으로 만들지 방향을 잡는다. → *AI 활용:* 생성형 AI로 초안 작화·영상을 빠르게 뽑고, 사람이 검수해 완성도를 맞춘다.
    - **콘텐츠 데이터 기획자 (경로 B):** OTT·웹툰 이용자 데이터를 들여다보며 무엇이 인기 있고 어떻게 마케팅할지 숫자로 판단하는 사람이다. 감이 아니라 데이터로 기획을 정한다. → *AI 활용:* BigQuery·Tableau로 시청·구독 데이터를 분석하고 AI로 트렌드를 예측한다.
    - **글로벌 IP 로컬라이제이션 매니저 (경로 C):** 한국 콘텐츠를 해외에 낼 때 번역·현지화 품질을 책임지는 사람이다. 나라별 정서에 맞게 다듬어 글로벌 서비스로 내보낸다. → *AI 활용:* AI가 1차 번역을 하면 사람이 감수하는 방식으로 다국어 품질을 관리한다.
    - **AI 창작 스타트업 창업가 (경로 D):** 자기 IP나 아이디어로 직접 콘텐츠 사업을 벌이는 사람이다. 웹툰을 짧은 애니로 바꾸는 등 새로운 형식에 도전한다. → *AI 활용:* 생성형 AI 제작 파이프라인으로 적은 인원·비용으로 콘텐츠를 만들어 낸다.
