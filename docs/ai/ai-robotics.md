...
**개편 방향:** 한 축에서는 정역학·동역학·열역학·유체역학의 **4대 역학**과 기계요소설계·생산공학·CAE(유한요소해석)를 전공의 절반으로 탄탄히 유지한다. 다른 축에서는 글로벌 패러다임이 생성형 AI→에이전트 AI→**Physical AI**로 전환되고 Physical AI 시장이 2026년 약 $110.8B(CAGR≈36%, 로봇·Embodied AI 포함 광의 추정; 좁은 정의 MarketsandMarkets 2026 $1.50B·2032 $15.24B)로 성장하는 흐름에 맞춰, NVIDIA의 '세 대의 컴퓨터'(학습-시뮬레이션-탑재추론) 구조와 PARC Lab의 실시간 제어·로봇 역량을 결합한다. **기계공학 기본기 위에 AI·로봇을 얹는** 순서가 핵심이다.
...
각 모듈 교과목이 **언제 개설되는지**(학년-학기)를 한눈에 보여준다. (예시·학기 미상 과목은 10-1 참조. M4·M5 기계공학 모듈은 학과 2027 신설 예정으로 개설 학기 미정)
...
!!! info "진출 직무 설명 — 이 직무는 어떤 일을 하나요?"
    ...
    - **로봇러닝(Physical AI) 엔지니어 (경로 A):** 로봇이 스스로 걷고 물건을 집는 동작을 데이터로 학습시켜 똑똑하게 움직이게 만드는 사람이다. 휴머노이드·협동로봇의 '두뇌'를 훈련시킨다. → *AI 활용:* 강화학습·모방학습으로 로봇 정책을 학습하고 Isaac Sim으로 시뮬레이션한다.
    - **로봇 SW·임베디드 엔지니어 (경로 B):** 로봇을 실제로 움직이는 제어 소프트웨어와 부품 펌웨어를 만드는 사람이다. 모터·센서가 제때 정확히 반응하도록 프로그램한다. → *AI 활용:* ROS2·실시간 제어에 AI 인식·판단 모듈을 연결해 로봇을 동작시킨다.
    - **기계설계·구조해석(CAE) 엔지니어 (경로 C):** 로봇·기계의 뼈대와 부품을 튼튼하고 안전하게 설계하고, 힘을 견디는지 컴퓨터로 미리 검증하는 사람이다. 전통 기계공학의 핵심 직무다. → *AI 활용:* 생성형 설계·시뮬레이션으로 구조 후보를 빠르게 만들고 해석 결과를 검토한다.
    - **생산기술·스마트팩토리 자동화 엔지니어 (경로 D):** 공장에서 제품이 잘 만들어지도록 가공·조립 공정을 짜고 로봇·설비로 자동화하는 사람이다. 제조 현장의 효율을 책임진다. → *AI 활용:* AI 비전·데이터로 공정 이상을 잡아내고 로봇 셀 자동화를 최적화한다.

### 10-5. 상위 수준 보완 권고

> 아래는 KAIST 기계공학·로봇, 한양대 로봇공학, POSTECH·GIST 로봇 등 AI·기계·로봇 특성화 **상위 비교군** 및 산업 표준 정렬을 위한 **보완 권고**다. **공식 교과를 대체하지 않으며**, 2027학년도 교과 개편 시 심의 의견·향후 개선 계획으로 활용한다. (기계공학 4대 역학·설계 50% 유지 원칙은 그대로 둔다.)
