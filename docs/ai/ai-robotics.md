...
| 모듈명 | 계층 | 핵심 역량·주제 | 학습 성과 | 대표 교과(AI로봇융합트랙 현행/신설 제안) |
| --- | --- | --- | --- | --- |
| AI 핵심·에이전트 기초 | 단과대학 공통 | AI/ML 기초, 생성형 AI·RAG·에이전트, AI 윤리·거버넌스 | ML 모델 구현, 에이전트 프로토타입, 로봇 AI 안전·윤리 분석 | 인공지능 개론, 인공지능 수학, 선형대수, 확률 및 통계, 생성형 AI와 에이전트(신설 예정/제안), AI 윤리와 거버넌스(신설 예정/제안) |
| 물리 AI·로보틱스 기초 | 학부 공통 | 센서·액추에이터, 실시간 제어 기초, 시뮬레이션 입문 | 센서 데이터 처리, 기본 제어 루프 구현, 시뮬레이터 환경 구성 | 디지털 논리와 회로, 오픈소스 HW, 컴퓨터구조, 물리 AI 개론(신설 예정/제안), 로보틱스·제어 기초(신설 예정/제안), 시뮬레이션 입문(신설 예정/제안) |
| AI 프로그래밍·데이터 | 학부 공통 | C++·Python, 데이터 파이프라인, Linux·Git 협업 | 로봇·AI 코드 작성, 데이터셋 구축, 협업 워크플로 운영 | C프로그래밍, 리눅스와C프로그래밍, 객체지향프로그래밍, 데이터베이스, 로봇 프로그래밍(신설 예정/제안) |
| **기계공학 핵심(4대 역학·재료)** | **학과 전공(전통)** | **정역학·동역학·열역학·유체역학, 고체역학, 기구학** | **역학 해석·운동 방정식 수립, 부재 강도 평가** | **정역학·동역학·열역학·유체역학·재료역학(신설 예정/제안)** |
| **기계설계·제조·CAE** | **학과 전공(전통)** | **기계요소설계, CAD/CAM, CAE(FEA·CFD), 생산·제조공학(CNC·적층제조)** | **기계 시스템 설계·구조해석, 가공 공정 설계** | **기계요소설계·CAD/CAM·유한요소해석(CAE)·생산공학(신설 예정/제안)** |
| 로봇 제어·기계 시스템 | 학과 전공(융합) | 모션제어·경로계획·매니퓰레이터, 액추에이터·메카트로닉스 | 제어 알고리즘 구현, 구동계 설계, 로봇 기구 설계 | 로봇 제어공학·모션플래닝·메카트로닉스(신설 예정/제안) |
| 임베디드·실시간 시스템 | 학과 전공 | ROS2/DDS, Realtime Linux/RTOS, CAN·MCU·FPGA | 실시간 제어 시스템 통합, 미들웨어 기반 노드 구현 | ROS2 미들웨어·실시간 임베디드·로봇 전장(신설 예정/제안) |
| 로봇러닝·시뮬레이션(Physical AI) | 학과 전공 | 강화학습·모방학습, 컴퓨터비전·SLAM, Isaac Sim·MuJoCo, Sim-to-Real | 로봇 정책 학습, 시뮬레이션 환경 구축, 실로봇 정책 이전 | 로봇 강화학습·모방학습·로봇 비전·SLAM·시뮬레이션·Sim-to-Real(신설 예정/제안) |
| 로봇 AI 캡스톤 | 학과 전공 | 학습→시뮬→탑재 통합, 산학 휴머노이드/협동로봇 프로젝트 | Physical AI 풀스택 프로토타입 완성·발표 | 로봇 캡스톤디자인·산학 프로젝트(신설 예정/제안) |
