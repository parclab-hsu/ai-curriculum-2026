...
AI기계로봇공학과는 **기계공학(4대 역학·기계설계·제조)을 굳건한 토대**로 삼고, 여기에 **제어·임베디드와 AI(강화학습·모방학습·컴퓨터비전)**를 융합해 휴머노이드·협동로봇·**Physical AI**를 구현하는 인재를 양성하는 **2027학년도 신설 예정** 학과입니다. 설계 원칙은 **전통 기계공학 50% : AI·로봇 융합 50%**의 균형으로, 'AI에만 치우치지 않은' 기계공학 정체성을 유지합니다.

**개편 방향:** 한 축에서는 정역학·동역학·열역학·유체역학의 **4대 역학**과 기계요소설계·생산공학·CAE(유한요소해석)를 전공의 절반으로 탄탄히 유지합니다. 다른 축에서는 글로벌 패러다임이 생성형 AI→에이전트 AI→**Physical AI**로 전환되고 Physical AI 시장이 2026년 약 $110.8B(CAGR≈36%, 로봇·Embodied AI 포함 광의 추정; 좁은 정의 MarketsandMarkets 2026 $1.50B·2032 $15.24B)로 성장하는 흐름에 맞춰, NVIDIA의 '세 대의 컴퓨터'(학습-시뮬레이션-탑재추론) 구조와 PARC Lab의 실시간 제어·로봇 역량을 결합합니다. **기계공학 기본기 위에 AI·로봇을 얹는** 순서가 핵심입니다.
...
!!! abstract "이 트랙과 향후 10년 고용"
    ...
    - **중국:** 산업로봇 200만 대(세계 54% 설치)·휴머노이드 2025 약 12,800대(세계 90%, 산업계 추정·언론 보도 기준)·AI/로봇 1조 위안 펀드로 휴머노이드 양산을 선도하며, 제조직 70%+ 대체 가능성이 Physical AI 인재 경쟁을 가속한다.
...
> 전형적 신입 요건(두 갈래): ① 기계공학 학사 + 4대 역학·CAD/CAE + 제어 기초, 또는 ② 컴퓨터/전자/제어 학사 + C++/Python + ROS — 본 학과는 **두 갈래를 한 전공 안에서 융합**합니다.

!!! tip "추가 보강 제안 (2026 개편 반영안 · 공식 교과 아님)"
    공식 교과를 대체하지 않는 **추가 보강 방향**입니다(신설/심화 제안).
    - **추가 기술트렌드:** Embodied AI · 로봇 파운데이션 모델 · Sim-to-Real
    - **추가 직무역량:** ROS2 · 동역학/제어 · 비전 · 안전 인증
    - **교육과정 보강(제안):** 2026 현행 AI로봇융합트랙과 2027 신설 학과를 분리 기술
...
> 전공 비중은 **전통 기계공학 50% : AI·로봇 융합 50%**를 원칙으로 합니다. 아래 1은 전통 축, 2~4는 융합 축입니다.
...
> 학문 분야 정체성: AI기계로봇공학과는 **기계공학(4대 역학·기계설계·제조)을 토대**로, 제어·임베디드와 강화학습·모방학습·컴퓨터비전을 결합해 휴머노이드·협동로봇·Physical AI를 구현하는 인재를 양성하는 **2027학년도 신설 예정** 학과입니다. 교육 목표도 **기계공학 50% : AI·로봇 50%**로 균형을 둡니다.

2027학년도 신설 예정 학과로서 기계공학 기본기 위에 PARC Lab의 Physical AI·실시간 제어·로봇 역량을 결합하고, NVIDIA '세 대의 컴퓨터'(학습-시뮬레이션-탑재추론) 구조를 융합 골격으로 삼아 측정 가능한 목표를 설정합니다.
...
> 출처: 한성대학교 AI기계로봇공학과(2027 신설 예정) 공식 교과과정([Engineering/4966](https://www.hansung.ac.kr/Engineering/4966/subview.do)) 기준, 확인일 2026-06-30. **현행 근거: AI로봇융합트랙·학과 2027 신설 예정**으로 학과 공식 교육과정 미확정. 구성 교과목은 현행 트랙 공식 교과목이며, 학과 미확정 보강(기계공학 모듈 포함)은 (제안), 개설 학년·학기 미상은 (미정)으로 표기. (전기=전공기초·전필=전공필수·전선=전공선택)
...
| --- | --- | --- | --- | --- | --- | --- |
| **M1** | AI 핵심·에이전트 기초 | 인공지능 개론(1-1·전기) · 인공지능 수학(1-2·전필) · 머신러닝(2-2·전필) · 생성형AI와에이전트(제안) | AI/ML 기초, 생성형 AI·에이전트, AI 윤리 | ML 모델 구현, 에이전트 프로토타입, 로봇 AI 윤리 분석 | 전공기초학습 | 공식·제안 |
| **M2** | AI 프로그래밍·데이터 | C프로그래밍(1-2·전선) · 리눅스와C프로그래밍(미정) · 객체지향프로그래밍(2-2·전선) · 데이터베이스(2-2·전선) | C·Python, 객체지향, 데이터베이스 | 로봇·AI 코드 작성, 데이터셋 구축 | 전공기초학습 | 공식·미정 |
| **M3** | 물리 AI·로보틱스·임베디드 기초 | 디지털 논리 및 회로(1-2·전선) · 오픈소스 HW(1-2·전선) · 컴퓨터구조(2-2·전선) · 시스템 프로그래밍(3-2·전선) | 디지털 논리·HW, 컴퓨터구조, 시스템 프로그래밍 | 센서·HW 처리, 시스템 레벨 코드 구현 | 전공기초학습 | 공식 |
| **M4** | 기계공학 핵심(4대 역학·재료) | 정역학·동역학(제안) · 열역학·유체역학(제안) · 재료역학(제안) | 정역학·동역학·열역학·유체역학, 재료역학 | 역학 해석·운동 방정식 수립, 부재 강도 평가 | M4-M5 상호보완 | 제안 |
| **M5** | 기계설계·제조·CAE | 기계요소설계(제안) · CAD·CAM(제안) · 유한요소해석CAE(제안) · 생산공학(제안) | 기계요소설계, CAD·CAM, CAE(FEA·CFD), 생산공학 | 기계 시스템 설계·구조해석, 가공 공정 설계 | M4-M5 상호보완 | 제안 |
| **M6** | 로봇 제어·로봇러닝·시뮬레이션 | 딥러닝(3-1·전필) · 컴퓨터비전(3-1·전필) · 강화학습(4-2·전선) · 로보틱스와 인공지능(3-2·전선) | 강화학습·딥러닝, 로봇 비전, 로보틱스·제어 | 로봇 정책 학습, 비전 인지, 제어 알고리즘 구현 | M1·M2·M3 선수 후 심화 | 공식 |
| **M7** | 로봇 AI 캡스톤 | AI프리캡스톤디자인(3-2·전필) · 기업연계 AI캡스톤디자인(3-2·전필) · 지능시스템 캡스톤디자인(4-1·전필) · 산학협력프로젝트(4-2·전선) | 학습→시뮬→탑재 통합, 산학 휴머노이드/협동로봇 프로젝트 | Physical AI 풀스택 프로토타입 완성·발표 | 기존 모듈학습 역량 검증 | 공식 |

### 10-2. 모듈형 교육과정 로드맵 (학년·학기)

각 모듈 교과목이 **언제 개설되는지**(학년-학기)를 한눈에 보여줍니다. (예시·학기 미상 과목은 10-1 참조. M4·M5 기계공학 모듈은 학과 2027 신설 예정으로 개설 학기 미정)

| 모듈 | 1-1 | 1-2 | 2-1 | 2-2 | 3-1 | 3-2 | 4-1 | 4-2 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| **M1** AI 핵심·에이전트 기초 | 인공지능 개론 | 인공지능 수학 | | 머신러닝 | | | | |
| **M2** AI 프로그래밍·데이터 | | C프로그래밍 | | 객체지향프로그래밍 · 데이터베이스 | | | | |
| **M3** 물리 AI·로보틱스·임베디드 기초 | | 디지털 논리 및 회로 · 오픈소스 HW | | 컴퓨터구조 | | 시스템 프로그래밍 | | |
| **M4** 기계공학 핵심(예시·학기 미정) | | | | | | | | |
| **M5** 기계설계·제조·CAE(예시·학기 미정) | | | | | | | | |
| **M6** 로봇 제어·로봇러닝·시뮬레이션 | | | | | 딥러닝 · 컴퓨터비전 | 로보틱스와 인공지능 | | 강화학습 |
| **M7** 로봇 AI 캡스톤 | | | | | | AI프리캡스톤디자인 · 기업연계 AI캡스톤디자인 | 지능시스템 캡스톤디자인 | 산학협력프로젝트 |

**모듈 흐름(요약 다이어그램):**

```mermaid
flowchart LR
  M1["M1 AI 핵심·에이전트 기초"] --> M6["M6 로봇 제어·로봇러닝·시뮬레이션"]
  M2["M2 AI 프로그래밍·데이터"] --> M6
  M3["M3 물리 AI·로보틱스·임베디드 기초"] --> M6
  M4["M4 기계공학 핵심"] --> M5["M5 기계설계·제조·CAE"] --> CAP["M7 로봇 AI 캡스톤"]
  M6 --> CAP
```

- **마이크로디그리:** 'M6 로봇 제어·로봇러닝·시뮬레이션', 'M3 물리 AI·로보틱스·임베디드 기초', 'M5 기계설계·제조·CAE' 각 모듈을 마이크로디그리로 인증할 수 있다.
- **타 학과 교차수강:** AI기계로봇 ↔ 미래모빌리티학과 간 Physical AI·실시간 제어·시뮬레이션·Edge AI 모듈을 공유하며, PARC Lab 도메인을 매개로 로봇 제어와 자율 이동체 역량을 교차 이수한다.

### 10-3. 학습자 진로 가이드

| 진로 분야 | 권장 모듈 조합 | 지향 |
| --- | --- | --- |
| 기계설계·해석 (전통) | M4 기계공학 핵심(4대 역학·재료) + M5 기계설계·제조·CAE + M6 로봇 제어·로봇러닝·시뮬레이션 | 기계설계 엔지니어·구조·CAE 해석 엔지니어 |
| 제조·생산기술 (전통) | M5 기계설계·제조·CAE + M4 기계공학 핵심(4대 역학·재료) + M3 물리 AI·로보틱스·임베디드 기초 | 생산기술 엔지니어·스마트팩토리 자동화 엔지니어 |
| 로봇 AI/제어 학습 (융합) | M1 AI 핵심·에이전트 기초 + M6 로봇 제어·로봇러닝·시뮬레이션 + M3 물리 AI·로보틱스·임베디드 기초 | 로봇 ML 엔지니어·강화·모방학습 제어 엔지니어 |
| 로봇 SW·임베디드 (융합) | M2 AI 프로그래밍·데이터 + M3 물리 AI·로보틱스·임베디드 기초 + M6 로봇 제어·로봇러닝·시뮬레이션 | 로봇 SW 아키텍트·모션제어·ROS2 엔지니어 |
...
!!! info "진출 직무 — 이런 전문가로 성장할 수 있어요"
    ...
    - **로봇러닝(Physical AI) 엔지니어 (경로 A):** 로봇이 스스로 걷고 물건을 집도록 데이터로 가르쳐 휴머노이드·협동로봇의 '두뇌'를 키워 내는 엔지니어입니다. 내가 훈련시킨 로봇이 어제는 못 하던 동작을 오늘 해내는, 살아 있는 것을 길러 내는 듯한 감동을 직접 경험할 수 있습니다. → *AI 활용 능력·역량:* 강화학습·모방학습으로 로봇 정책을 학습하고 Isaac Sim으로 검증하는 **로봇러닝(Physical AI) 개발 역량**.
    - **로봇 SW·임베디드 엔지니어 (경로 B):** 상상 속 로봇을 실제로 움직이게 하는 제어 소프트웨어와 펌웨어를 짜는, 하드웨어에 생명을 불어넣는 엔지니어입니다. 내 코드에 맞춰 모터와 센서가 정확히 반응하며 로봇이 눈앞에서 움직일 때의 성취감이 매력입니다. → *AI 활용 능력·역량:* ROS2·실시간 제어에 AI 인식·판단 모듈을 연결해 로봇을 동작시키는 **로봇 SW·임베디드 개발 역량**.
    - **기계설계·구조해석(CAE) 엔지니어 (경로 C):** 로봇·기계의 뼈대와 부품을 튼튼하고 안전하게 설계하고 컴퓨터로 미리 검증하는, 전통 기계공학의 핵심을 지키는 설계자입니다. 내가 그린 구조가 실제 제품이 되어 오랜 시간 힘을 견뎌 내는 신뢰의 보람을 누릴 수 있습니다. → *AI 활용 능력·역량:* 생성형 설계·시뮬레이션으로 구조 후보를 빠르게 만들고 해석 결과를 검토하는 **AI 기반 기계설계·CAE 역량**.
    - **생산기술·스마트팩토리 자동화 엔지니어 (경로 D):** 공장 전체가 로봇과 설비로 척척 돌아가도록 공정을 설계하고 자동화하는, 제조 현장의 효율을 책임지는 엔지니어입니다. 내가 짠 라인이 불량 없이 제품을 쏟아 내며 눈에 보이는 성과로 증명되는 짜릿함을 경험할 수 있습니다. → *AI 활용 능력·역량:* AI 비전·데이터로 공정 이상을 잡아내고 로봇 셀 자동화를 최적화하는 **스마트팩토리 자동화 역량**.

### 10-5. 상위 수준 보완 권고

> 아래는 KAIST 기계공학·로봇, 한양대 로봇공학, POSTECH·GIST 로봇 등 AI·기계·로봇 특성화 **상위 비교군** 및 산업 표준 정렬을 위한 **보완 권고**입니다. **공식 교과를 대체하지 않으며**, 2027학년도 교과 개편 시 심의 의견·향후 개선 계획으로 활용합니다. (기계공학 4대 역학·설계 50% 유지 원칙은 그대로 둡니다.)

| 보완 영역 | 반영 위치 | 추가하면 좋은 내용 | 기대 효과 |
| --- | --- | --- | --- |
| ROS2 미들웨어 정식 교과화 | M2, M3 | DDS·QoS·rclcpp/rclpy·Nav2·tf2를 흩어진 도구가 아닌 독립 과목으로 편성, colcon·launch·실시간 노드 실습 | 거의 전 로봇 공고 공통 요건을 정규 학점으로 보장, 입사 즉시 투입 |
| 로봇 동역학·고급제어 | M4, M6 | 다관절 로봇 동역학(라그랑주·뉴턴-오일러), 토크기반 제어·임피던스/힘제어·최적제어(MPC) | 비교군 기계공학 강점인 동역학 기반기 확보, 휴머노이드 전신 제어 대비 |
| 매니퓰레이션·모션플래닝 | M6 | MoveIt2·OMPL 경로계획, 그래스핑·6D 자세추정(비전), 충돌회피·역기구학 핸즈온 | 협동로봇·매니퓰레이터 실무(두산·뉴로메카형) 직결, 비전-제어 통합 역량 |
| 로봇 파운데이션 모델(VLA) | M6, M1 | VLA(RT-2·RT-X·OpenVLA)·Isaac GR00T·π0 등 정책 학습, language-conditioned manipulation 실습 | Embodied AI 최신 패러다임 반영, 삼성 RFM·NVIDIA GR00T 채용군 정렬 |
| Sim-to-Real 파이프라인 정규화 | M6, M7 | Isaac Lab 도메인 랜덤화·system identification, MuJoCo→실로봇 이전·zero-shot 전이 평가 | 시뮬-실물 격차 정량 관리, 캡스톤 성공률·재현성 제고 |
| 로봇 안전·표준 인증 | M5, M7 | ISO 10218-1/2·ISO/TS 15066(협동로봇 PFL·속도/힘 한계), 안전평가·리스크 어세스먼트 | 산업 현장 배치 필수 안전 규격 내재화, 협동로봇 상용화 적합성 확보 |
