...
4. **실시간 임베디드·미들웨어 역량:** ROS2/DDS·Realtime Linux/RTOS와 임베디드(CAN·MCU·FPGA) 환경에서 실시간 제어 시스템을 통합할 수 있다. (신설 학과로서 1기부터 K-휴머노이드 연합·제4차 지능형로봇 기본계획 산학 과제와 연계해 현장에 즉시 투입할 수 있는 인재로 육성한다.)
...
- **경로 C — 모션제어·액추에이터 펌웨어 엔지니어:** 1학년 C++·Python + 기계·전자·제어 기초 → 2학년 로봇 제어공학 + 기계설계·CAD → 3학년 모션플래닝 + 실시간 임베디드(CAN/MCU·STM32) → 4학년 로봇 전장 + 로봇 AI 캡스톤(두산로보틱스·로보티즈형 매니퓰레이터 모션제어·다이나믹셀 펌웨어 프로젝트) → 모션제어 SW 엔지니어·액추에이터 펌웨어 엔지니어로 진출.

- **경로 D — Physical AI 로봇 연구원(대학원 진학):** 1학년 C++·Python·ROS2 기초 + AI/ML 입문 → 2학년 물리 AI 개론 + 로봇 비전·SLAM → 3학년 로봇 강화학습·모방학습 + MuJoCo 시뮬레이션 → 4학년 Sim-to-Real + 로봇 AI 캡스톤(삼성 로봇 파운데이션 모델·K-휴머노이드 연합형 정책학습 연구) → 대학원 진학·Physical AI 로봇 연구원으로 진출.
