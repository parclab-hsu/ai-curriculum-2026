...
| **기계공학 핵심(4대 역학·재료)** | **학과 전공(전통)** | **정역학·동역학·열역학·유체역학, 고체역학, 기구학** | **역학 해석·운동 방정식 수립, 부재 강도 평가** | **정역학, 동역학, 열역학, 유체역학, 재료역학** |
| **기계설계·제조·CAE** | **학과 전공(전통)** | **기계요소설계, CAD/CAM, CAE(FEA·CFD), 생산·제조공학(CNC·적층제조)** | **기계 시스템 설계·구조해석, 가공 공정 설계** | **기계요소설계, CAD/CAM, 유한요소해석(CAE), 생산공학** |
| 로봇 제어·기계 시스템 | 학과 전공(융합) | 모션제어·경로계획·매니퓰레이터, 액추에이터·메카트로닉스 | 제어 알고리즘 구현, 구동계 설계, 로봇 기구 설계 | 로봇 제어공학, 모션플래닝, 메카트로닉스 |
