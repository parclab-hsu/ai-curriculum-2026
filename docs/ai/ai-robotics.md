...
> 출처: 한성대학교 AI기계로봇공학과(예시) 공식 교과과정([Engineering/4966](https://www.hansung.ac.kr/Engineering/4966/subview.do)) 기준, 확인일 2026-06-30. **현행 근거: AI로봇융합트랙·학과 2027 신설 예정**으로 학과 공식 교육과정 미확정. 구성 교과목은 현행 트랙 공식 교과목이며, 학과 미확정 보강(기계공학 모듈 포함)은 (예시), 개설 학년·학기 미상은 (학기 미상)으로 표기. (전기=전공기초·전필=전공필수·전선=전공선택)

| 모듈 | 모듈명 | 구성 교과목 (학년-학기·이수구분) | 모듈 설명 | 모듈 학습성과 | 모듈 간 관계 |
| --- | --- | --- | --- | --- | --- |
| **M1** | AI 핵심·에이전트 기초 | 인공지능 개론(1-1·전기) · 인공지능 수학(1-2·전필) · 머신러닝(2-2·전필) · 생성형AI와에이전트(예시) | AI/ML 기초, 생성형 AI·에이전트, AI 윤리 | ML 모델 구현, 에이전트 프로토타입, 로봇 AI 윤리 분석 | 전공기초학습 |
| **M2** | AI 프로그래밍·데이터 | C프로그래밍(1-2·전선) · 리눅스와C프로그래밍(학기 미상) · 객체지향프로그래밍(2-2·전선) · 데이터베이스(2-2·전선) | C·Python, 객체지향, 데이터베이스 | 로봇·AI 코드 작성, 데이터셋 구축 | 전공기초학습 |
| **M3** | 물리 AI·로보틱스·임베디드 기초 | 디지털 논리 및 회로(1-2·전선) · 오픈소스 HW(1-2·전선) · 컴퓨터구조(2-2·전선) · 시스템 프로그래밍(3-2·전선) | 디지털 논리·HW, 컴퓨터구조, 시스템 프로그래밍 | 센서·HW 처리, 시스템 레벨 코드 구현 | 전공기초학습 |
| **M4** | 기계공학 핵심(4대 역학·재료) | 정역학·동역학(예시) · 열역학·유체역학(예시) · 재료역학(예시) | 정역학·동역학·열역학·유체역학, 재료역학 | 역학 해석·운동 방정식 수립, 부재 강도 평가 | M4-M5 상호보완 |
| **M5** | 기계설계·제조·CAE | 기계요소설계(예시) · CAD·CAM(예시) · 유한요소해석CAE(예시) · 생산공학(예시) | 기계요소설계, CAD·CAM, CAE(FEA·CFD), 생산공학 | 기계 시스템 설계·구조해석, 가공 공정 설계 | M4-M5 상호보완 |
| **M6** | 로봇 제어·로봇러닝·시뮬레이션 | 딥러닝(3-1·전필) · 컴퓨터비전(3-1·전필) · 강화학습(4-2·전선) · 로보틱스와 인공지능(3-2·전선) | 강화학습·딥러닝, 로봇 비전, 로보틱스·제어 | 로봇 정책 학습, 비전 인지, 제어 알고리즘 구현 | M1·M2·M3 선수 후 심화 |
| **M7** | 로봇 AI 캡스톤 | AI프리캡스톤디자인(3-2·전필) · 기업연계 AI캡스톤디자인(3-2·전필) · 지능시스템 캡스톤디자인(4-1·전필) · 산학협력프로젝트(4-2·전선) | 학습→시뮬→탑재 통합, 산학 휴머노이드/협동로봇 프로젝트 | Physical AI 풀스택 프로토타입 완성·발표 | 기존 모듈학습 역량 검증 |
