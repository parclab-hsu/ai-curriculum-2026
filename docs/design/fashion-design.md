...
!!! info "진출 직무 설명 — 이 직무는 어떤 일을 하나요?"
    ...
    - **어패럴 디자이너 (경로 A):** 옷의 콘셉트를 잡고 실루엣·색·소재를 정해 한 시즌의 옷을 디자인하는 일을 한다. 스케치부터 샘플 확인까지 컬렉션 전체를 이끈다. → *AI 활용:* 생성형 AI(Midjourney·Firefly)로 무드보드와 디자인 시안을 빠르게 여러 벌 만들어 보고 그중 좋은 방향을 골라 발전시킨다.
    - **3D 패션 디자이너 (경로 B):** 실물 옷을 만들기 전에 컴퓨터 안에서 옷을 3D로 지어 핏과 드레이프를 미리 확인하는 일을 한다. 디지털 샘플로 수정을 반복해 실물 제작 횟수를 줄인다. → *AI 활용:* CLO 3D·가상 피팅 도구로 아바타에 옷을 입혀 시뮬레이션하고, 생성형 AI로 텍스처·패턴을 채워 넣는다.
    - **텍스타일/프린트 디자이너 (경로 C):** 옷에 쓰이는 원단의 무늬·프린트·색 조합을 디자인하는 일을 한다. 시즌 트렌드에 맞는 패턴과 소재의 느낌을 만들어 낸다. → *AI 활용:* 생성형 AI로 프린트·텍스처 패턴을 대량 생성하고, 색채 데이터를 분석해 유행할 색 조합을 고른다.
    - **디지털 패션 콘텐츠 창업가/연구자 (경로 D):** 실물 없이 화면에서만 존재하는 가상 의상·디지털 패션 콘텐츠를 기획하거나, 관련 기술을 연구하는 일을 한다. 게임·SNS·가상세계용 옷을 만들거나 대학원에서 새 방법을 탐구한다. → *AI 활용:* 3D·생성형 AI로 가상 의상과 디지털 룩북을 제작하고, 새로운 AI 디자인 워크플로를 실험·검증한다.
