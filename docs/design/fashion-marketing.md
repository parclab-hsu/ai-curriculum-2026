...
!!! info "진출 직무 설명 — 이 직무는 어떤 일을 하나요?"
    ...
    - **AI 기반 패션 MD/바이어 (경로 A):** 어떤 상품을 얼마나 매입·기획해 언제·얼마에 팔지 정하는 **상품기획자(MD)이자 구매담당(바이어)**입니다. 시즌별 상품 구성과 발주 물량·가격을 결정합니다. → *AI 활용:* 판매·재고·트렌드 데이터로 **수요를 예측**하고, 추천 모델로 잘 팔릴 상품과 적정 발주량을 계산해 재고 과잉·품절을 줄입니다.
    - **디지털 콘텐츠 마케터 (경로 B):** 브랜드의 SNS·광고·영상 콘텐츠를 기획·제작해 고객을 끌어오는 **마케터**입니다. 캠페인 메시지와 채널을 설계합니다. → *AI 활용:* **생성형 AI로 카피·이미지·영상 시안**을 빠르게 만들고, 반응 데이터를 분석해 콘텐츠를 개선합니다.
    - **AI 그로스·CRM 마케터 (경로 C):** 이미 확보한 고객의 재구매·이탈을 관리하며 매출 성장(그로스)을 만드는 **마케터**입니다. 고객을 세분화해 개인화 메시지를 보냅니다. → *AI 활용:* 고객 데이터로 **이탈 예측·개인화 추천**을 하고, A/B 테스트로 전환율을 높입니다.
    - **패션 커머스 창업가/브랜드 매니저 (경로 D):** 자체 브랜드(D2C)를 만들어 상품 기획부터 판매·운영까지 총괄하는 **창업가·브랜드 책임자**입니다. → *AI 활용:* 수요예측·콘텐츠 생성·CRM을 **AI 도구로 소규모 팀처럼 운영**해 빠르게 브랜드를 키웁니다.
