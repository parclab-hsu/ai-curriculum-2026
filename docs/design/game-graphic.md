...
!!! info "진출 직무 설명 — 이 직무는 어떤 일을 하나요?"
    ...
    - **게임 컨셉 아티스트·캐릭터 원화가 (경로 A):** 게임 속 캐릭터·배경·세계관의 첫 그림(원화)을 그려 아트 방향을 잡아주는 일을 한다. → *AI 활용:* Stable Diffusion·Midjourney로 컨셉·텍스처 시안을 여럿 만들어 아이디어를 빠르게 탐색한다.
    - **3D 게임 아티스트·테크니컬 아티스트 (경로 B):** 캐릭터·배경을 3D 모델로 만들고 게임 엔진에서 잘 돌아가게 다듬어 넣는 일을 한다. → *AI 활용:* AI 에셋 생성과 자동화 파이프라인으로 모델·텍스처 제작 시간을 줄인다.
    - **게임 UI 디자이너·리얼타임 아티스트 (경로 C):** 체력바·메뉴 등 게임 화면 인터페이스를 보기 좋고 쓰기 편하게 설계해 엔진에 붙이는 일을 한다. → *AI 활용:* 인게임 UI 시안을 생성해 초안을 빨리 만들고 실시간 화면에서 비교한다.
    - **생성형 AI 아티스트(R&D) (경로 D):** 게임사 안에서 AI 아트 도구·워크플로우를 연구하고 제작팀이 쓸 수 있게 다듬어 지원하는 일을 한다. → *AI 활용:* LoRA 커스텀 학습·이미지 투 이미지 등으로 스튜디오 전용 에셋 생성 파이프라인을 만든다.

### 10-5. 상위 수준 보완 권고

> 아래는 청강문화산업대 게임전공·계원예대·홍익대 게임학부 등 게임그래픽·실시간 3D 특성화 **상위 비교군** 및 산업 표준(Epic/Unity·Unreal/Unity 아트 파이프라인) 정렬을 위한 **보완 권고**다. **공식 교과를 대체하지 않으며**, 2027학년도 교과 개편 시 심의 의견·향후 개선 계획으로 활용한다.
