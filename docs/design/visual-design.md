...
!!! info "진출 직무 설명 — 이 직무는 어떤 일을 하나요?"
    ...
    - **브랜드 디자이너 (경로 A):** 로고·색·서체 같은 브랜드의 얼굴을 만들고, 여러 매체에서 일관되게 보이도록 규칙을 정리하는 일을 한다. → *AI 활용:* Midjourney 등으로 브랜드 시안을 여러 개 빠르게 만들고 어울리는 방향을 골라 다듬는다.
    - **편집 디자이너·아트디렉터 (경로 B):** 잡지·책·광고 지면의 글과 이미지를 읽기 좋게 배치하고, 촬영·시안 전체의 톤을 정해 이끄는 일을 한다. → *AI 활용:* 레이아웃·이미지 시안을 자동으로 생성해 초안을 빨리 잡고 방향을 비교한다.
    - **인포그래픽 디자이너·데이터 비주얼라이저 (경로 C):** 복잡한 숫자·정보를 한눈에 이해되는 그림과 차트로 바꿔 전달하는 일을 한다. → *AI 활용:* 데이터에서 핵심을 요약하거나 차트 시안을 뽑아 표현 아이디어를 넓힌다.
    - **프롬프트 디자이너·생성형 UX 설계자 (경로 D):** 화면을 하나하나 그리는 대신, AI가 좋은 결과를 내도록 지시문(프롬프트)과 디자인 규칙·시스템을 설계하는 일을 한다. → *AI 활용:* 프롬프트와 컴포넌트 규칙을 짜서 일관된 화면·에셋이 자동으로 만들어지게 한다.
