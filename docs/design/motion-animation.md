...
!!! info "진출 직무 설명 — 이 직무는 어떤 일을 하나요?"
    ...
    - **모션그래픽 디자이너 (경로 A):** 글자·도형·영상이 움직이는 짧은 영상을 만들어 광고·콘텐츠에서 메시지를 리듬감 있게 전달하는 일을 한다. → *AI 활용:* 텍스트로 영상 시안을 생성하거나 반복 작업을 자동화해 편집 속도를 높인다.
    - **애니메이터 (경로 B):** 캐릭터와 배경에 움직임과 연기를 불어넣어 2D·3D 애니메이션 장면을 만드는 일을 한다. → *AI 활용:* 컨셉아트·중간 프레임을 생성해 밑그림을 빠르게 잡고 표현을 넓힌다.
    - **브랜디드 콘텐츠 PD·광고영상 디렉터 (경로 C):** 브랜드 캠페인 영상을 기획하고 연출·제작 전 과정을 이끌어 이야기가 담긴 콘텐츠를 완성하는 일을 한다. → *AI 활용:* 스토리보드·시안 영상을 빠르게 만들어 기획 방향을 미리 확인하고 조율한다.
    - **버추얼 프로덕션 아티스트·AI 테크 디렉터(TD) (경로 D):** 언리얼엔진 같은 실시간 도구로 가상 세트를 만들고 촬영·합성 파이프라인을 설계·운영하는 기술 담당 일을 한다. → *AI 활용:* 실시간 3D·합성·후반작업 도구를 연결해 제작 흐름을 자동화하고 문제를 해결한다.
