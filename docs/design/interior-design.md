...
인테리어디자인트랙은 주거·상업 공간의 **공간기획 → 설계(평면/입면/상세/투시) → 재료·마감 → 시공관리**를 다루는 트랙입니다. 전통적으로 AutoCAD·SketchUp 기반 2D/3D 설계 역량이 핵심이었으나, **생성형 AI 렌더링·BIM·디지털 트윈·프롭테크**의 확산으로 직무가 재편되고 있습니다.

**AI 융합 개편 방향**: '인테리어디자이너'에서 **'AI 공간 디자이너 / 공간 데이터 디자이너'**로 직무를 확장합니다. 생성형 AI 시각화, AI 상담·견적 자동화, BIM 기반 데이터 설계를 표준 역량으로 통합하여 기획·시각화·정밀설계를 AI와 협업하는 인재를 양성합니다.
...
> 학문 분야 정체성: 인테리어디자인트랙은 인간 행동과 공간 경험에 대한 통찰을 바탕으로 거주·상업·공공 공간을 기획·설계·구현하는 공간 디자인 전문가를 양성합니다.
...
>
> ※ 공식 교과목 로드맵이 PDF·이미지로만 제공되어 **WebFetch 일부 실패** — 학년·학기가 확인되지 않은 교과목은 (학기 미상)으로 표기합니다.
...
!!! info "진출 직무 — 이런 전문가로 성장할 수 있어요"
    ...
    - **인테리어 디자이너 (경로 A):** 주거·상업 공간의 평면과 마감, 조명을 기획해 머릿속 상상을 실제로 살 수 있는 공간으로 완성하는 공간 창작자입니다. 내가 그린 설계안이 시공을 거쳐 누군가의 매일이 머무는 집과 매장으로 실현되는 성취를 직접 경험할 수 있습니다. → *AI 활용 능력·역량:* 미드저니·RoomGPT로 컨셉 이미지를 즉시 생성해 클라이언트와 방향을 맞추는 **생성형 AI 공간 시각화 역량**.
    - **3D 비주얼라이저 (경로 B):** 아직 존재하지 않는 공간을 사진과 구분되지 않을 만큼 실감 나는 3D 투시도로 먼저 보여 주는 시각화 전문가입니다. 내가 만든 이미지 한 장이 클라이언트의 결정을 이끌고 프로젝트를 성사시키는 결정적 순간을 경험할 수 있습니다. → *AI 활용 능력·역량:* 생성형 AI 렌더링으로 시안 수십 종을 단시간에 만들어 비교·선별하는 **AI 렌더링 자동화 역량**.
    - **공간 데이터 컨설턴트·스마트홈 기획자 (경로 C):** 사람들이 공간을 어떻게 쓰는지(동선·머무는 자리)를 데이터로 읽어 더 편한 공간과 스마트홈 서비스를 설계하는 미래형 기획자입니다. '느낌'이 아닌 데이터로 공간을 제안해 설득력 있는 근거를 갖춘 전문가로 인정받는 보람을 누릴 수 있습니다. → *AI 활용 능력·역량:* 동선·점유 데이터를 AI로 분석해 설계 근거로 전환하는 **공간 데이터 분석·의사결정 역량**.
    - **BIM 공간 설계자·디지털 트윈 공간 엔지니어 (경로 D):** 건물 정보를 담은 3D 모델(BIM)로 설계하고 실제 공간을 그대로 복제한 디지털 트윈으로 운영까지 관리하는, 공공·건설 현업이 가장 필요로 하는 인재입니다. 2030년 공공공사 BIM 전면 의무화의 흐름을 타고 설계-시공-운영을 잇는 핵심 전문가로 성장할 수 있습니다. → *AI 활용 능력·역량:* Revit·BIM 데이터를 AI와 연동해 설계 충돌·오류를 사전에 걸러내는 **BIM·디지털 트윈 데이터 설계 역량**.

### 10-5. 상위 수준 보완 권고

> 아래는 홍익대·건국대·국민대 실내건축/공간디자인 등 인테리어·공간디자인 특성화 **상위 비교군** 및 산업 표준 정렬을 위한 **보완 권고**입니다. **공식 교과를 대체하지 않으며**, 2027학년도 교과 개편 시 심의 의견·향후 개선 계획으로 활용합니다.
