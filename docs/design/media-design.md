...
!!! info "진출 직무 설명 — 이 직무는 어떤 일을 하나요?"
    ...
    - **UX/프로덕트 디자이너 (경로 A):** 앱·웹 같은 디지털 제품에서 사용자가 편하게 쓸 수 있도록 화면과 사용 흐름을 설계하는 일을 한다. 사용자 조사로 문제를 찾아 개선한다. → *AI 활용:* AI로 사용자 데이터·행동 패턴을 분석해 어디를 고칠지 근거를 찾고, 생성형 AI로 화면 시안을 빠르게 만들어 본다.
    - **인터랙티브 미디어 아티스트 (경로 B):** 관객이 움직이거나 만지면 반응하는 미디어아트·전시 작품을 만드는 일을 한다. 빛·영상·소리로 몰입 경험을 연출한다. → *AI 활용:* 생성형 AI로 배경·에셋을 만들고, 실시간 3D 엔진과 결합해 관객 반응형 생성 콘텐츠를 구현한다.
    - **전시·공간 미디어 디렉터 (경로 C):** 건물 벽면(미디어 파사드)이나 전시 공간에 들어갈 대형 영상·체험 콘텐츠를 기획하고 연출하는 일을 한다. 공간 전체를 미디어로 채운다. → *AI 활용:* 생성형 AI로 대형 영상 에셋을 제작하고, 센서 데이터와 연동해 공간이 관객에 반응하도록 연출한다.
    - **디지털 콘텐츠 서비스 기획자/창업가 (경로 D):** 새로운 디지털 콘텐츠나 서비스를 기획해 시제품(MVP)을 만들고 사용자 반응을 검증하는 일을 한다. 아이디어를 실제 제품·창업으로 발전시킨다. → *AI 활용:* 생성형 AI로 콘텐츠 시제품을 빠르게 만들어 보고, 사용성 데이터를 분석해 서비스 방향을 다듬는다.
