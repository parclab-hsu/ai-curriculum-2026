...
## 8. 교육 목표 (예시)
> 학문 분야 정체성: 미디어디자인트랙은 인터랙티브 미디어·UX/UI·뉴미디어 경험을 설계하는 분야로, 사용자와 콘텐츠가 실시간으로 상호작용하는 디지털 경험을 AI와 결합하여 창의적으로 구현하는 디자이너를 양성합니다.
...
!!! info "진출 직무 — 이런 전문가로 성장할 수 있어요"
    ...
    - **UX/프로덕트 디자이너 (경로 A):** 앱·웹 같은 디지털 제품에서 사용자가 편하게 쓰도록 화면과 사용 흐름을 설계하는 전문가입니다. 수백만 명이 매일 쓰는 서비스의 불편을 내가 찾아내 고치고, 그 개선이 지표로 증명되는 성취를 경험할 수 있습니다. → *AI 활용 능력·역량:* AI로 사용자 데이터·행동 패턴을 분석해 개선 근거를 찾고 생성형 AI로 화면 시안을 신속히 도출하는 **데이터 근거 UX 설계 역량**.
    - **인터랙티브 미디어 아티스트 (경로 B):** 관객이 움직이거나 만지면 반응하는 미디어아트·전시 작품을 창조하는 예술가입니다. 빛·영상·소리로 공간을 채우고, 내 작품 앞에서 사람들이 감탄하며 몰입하는 순간을 직접 연출하는 짜릿함을 누릴 수 있습니다. → *AI 활용 능력·역량:* 생성형 AI로 배경·에셋을 제작하고 실시간 3D 엔진과 결합해 관객 반응형 콘텐츠를 구현하는 **생성형 실시간 인터랙션 구현 역량**.
    - **전시·공간 미디어 디렉터 (경로 C):** 건물 벽면(미디어 파사드)이나 전시 공간을 대형 영상·체험 콘텐츠로 채우고 연출하는 공간 연출가입니다. 도시의 랜드마크와 대형 전시를 내 손으로 미디어의 무대로 바꾸는 스케일 큰 작업을 이끌 수 있습니다. → *AI 활용 능력·역량:* 생성형 AI로 대형 영상 에셋을 제작하고 센서 데이터와 연동해 공간을 관객 반응형으로 연출하는 **AI 공간 미디어 연출 역량**.
    - **디지털 콘텐츠 서비스 기획자/창업가 (경로 D):** 새로운 디지털 콘텐츠·서비스를 기획해 시제품(MVP)을 만들고 사용자 반응으로 검증하는 창업가형 인재입니다. 내 아이디어를 실제 제품과 창업으로 키워 시장에 직접 던져 보는 도전을 할 수 있습니다. → *AI 활용 능력·역량:* 생성형 AI로 콘텐츠 시제품을 빠르게 만들고 사용성 데이터를 분석해 서비스를 다듬는 **AI 기반 서비스 프로토타이핑 역량**.

### 10-5. 상위 수준 보완 권고

> 아래는 홍익대·국민대 미디어아트, 카이스트 문화기술(CT) 등 인터랙티브 미디어아트 특성화 **상위 비교군** 및 산업 표준 정렬을 위한 **보완 권고**입니다. **공식 교과를 대체하지 않으며**, 2027학년도 교과 개편 시 심의 의견·향후 개선 계획으로 활용합니다.
