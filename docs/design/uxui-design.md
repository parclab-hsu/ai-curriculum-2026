...
UX/UI디자인트랙은 디지털 제품·서비스의 **사용자 경험(UX)과 인터페이스(UI), 프로덕트 디자인**을 다룹니다. Figma·디자인시스템 기반 화면 설계가 핵심이었으나, **생성형 AI·에이전트 AI**가 디자이너 워크플로를 재편하며 직무가 '화면 제작'에서 **'문제 정의·시스템 설계·AI 오케스트레이션'**으로 이동하고 있습니다.

**AI 융합 개편 방향**: 단순 UI 제작은 AI에 흡수되므로, **'AI 활용 프로덕트 디자이너 / 디자인 엔지니어'**로 직무를 재정의합니다.
...
> 학문 분야 정체성: UX/UI디자인트랙은 사용자 조사와 인터랙션 설계를 바탕으로 디지털 제품·서비스의 화면과 경험을 설계·검증하는 UX/UI 디자인 전문가를 양성합니다.
...
!!! info "진출 직무 — 이런 전문가로 성장할 수 있어요"
    ...
    - **프로덕트 디자이너 (경로 A):** 앱·웹 서비스의 화면과 사용 흐름 전체를 설계해, 수많은 사용자가 매일 쓰는 제품의 경험을 직접 빚어내는 디자이너입니다. 내가 그린 화면이 실제 서비스로 출시돼 사용자의 손끝에서 살아 움직이는 성취를 경험할 수 있습니다. → *AI 활용 능력·역량:* 생성형 AI로 UI 시안·아이콘·카피를 빠르게 발산하고 사용성 기준으로 큐레이션하는 **AI 활용 프로덕트 디자인 역량**.
    - **UX 리서처 (경로 B):** 사용자를 조사·인터뷰·데이터 분석해 '무엇이 불편한지'를 누구보다 먼저 밝혀내고, 제품이 나아갈 방향을 근거로 제시하는 인사이트의 발견자입니다. 내 리서치 한 줄이 제품 전략을 바꾸는 결정적 순간을 만들 수 있습니다. → *AI 활용 능력·역량:* 사용성 데이터·리뷰를 AI로 분석·요약해 인사이트를 신속히 도출하는 **데이터 기반 UX 리서치 역량**.
    - **디자인 엔지니어 / 디자인 시스템 엔지니어 (경로 C):** 디자인과 개발의 경계에서 디자인을 실제 코드·컴포넌트로 연결하고, 팀 전체가 쓰는 재사용 가능한 디자인 시스템을 세우는 희소한 전문가입니다. 디자인과 코드를 모두 다루는 몇 안 되는 역할로서 제품의 뼈대를 만드는 성취를 누릴 수 있습니다. → *AI 활용 능력·역량:* v0·Figma MCP로 디자인→코드 핸드오프를 자동화하고 토큰·컴포넌트를 동기화하는 **디자인 엔지니어링 역량**.
    - **공간 인터랙션·XR 프로덕트 디자이너 (경로 D):** 키오스크·전시·XR처럼 공간과 디지털이 결합된 새로운 인터페이스를 설계해, 화면 너머로 경험의 무대를 넓히는 미래형 디자이너입니다. 아직 정답이 없는 영역을 내가 먼저 개척하는 창작의 짜릿함을 경험할 수 있습니다. → *AI 활용 능력·역량:* 생성형 AI로 공간·XR 콘텐츠와 인터랙션 프로토타입을 제작하는 **공간·XR 인터랙션 디자인 역량**.

### 10-5. 상위 수준 보완 권고

> 아래는 국민대·홍익대(경험·시각디자인), 카네기멜런 HCI·Google UX 등 UX/UI·제품디자인 특성화 **상위 비교군** 및 산업 표준 정렬을 위한 **보완 권고**입니다. **공식 교과를 대체하지 않으며**, 2027학년도 교과 개편 시 심의 의견·향후 개선 계획으로 활용합니다.
