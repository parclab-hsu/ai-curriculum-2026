...
**Gartner 전략 트렌드와 산업 동향을 종합하면, 2024년에는 생성형 AI 활용이 빠르게 확산됐고, 2025년에는 에이전트 AI가 주요 관심사로 부상했으며, 2026년에는 Physical AI·멀티에이전트·도메인 특화 모델의 중요성이 커지고 있다.** 같은 기간 직무 시장에서도 `AI 활용 역량`에 대한 요구가 강화되는 흐름이 확인된다. 이러한 변화는 PARC Lab의 도메인(Physical AI·로봇·모빌리티 실시간 제어)과 관련성이 높다.

> 다만 이 흐름은 기존 기술이 순차적으로 사라졌다는 뜻이 아니다. 생성형 AI·에이전트 AI·Physical AI가 함께 발전하는 가운데, 산업과 직무 시장의 관심이 점차 **물리 세계 적용과 도메인 특화 역량으로 확장되고 있음**을 보여주는 해석으로 이해하면 된다.
